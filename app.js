...
function applyLanguage(lang,persist=true){
  ...
  document.title=state.lang==='en'?'Albion Europe Market Scanner v5.5.5 — Trading and Refining':'Albion Europe Market Scanner v5.5.5 — Handel i rafinacja';
...
async function scan(opts={}){
  ...
  const cfg=cfgFromUI();state.opportunities=calcOpportunities(byItem,map,el('mode').value,cfg);markRouteRanks(state.opportunities);render();renderScanDiagnostics();
...
async function exportDB(){
  ...
  a.href=u;a.download=`albion-local-db-v5_5_5-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);
}
...
async function init(){
  ...
  if('serviceWorker' in navigator)navigator.serviceWorker.register('./service-worker.js?v=5.5.1').catch(()=>{});
...
function exportCsv(){
  ...
  const blob=new Blob(['\ufeff'+lines.join('\n')],{type:'text/csv;charset=utf-8'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='albion_europe_bulk_trade_v5_5_5.csv';a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);
}
init();
})();
