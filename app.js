...
function applyLanguage(lang,persist=true){
  ...
  document.title=state.lang==='en'?'Albion Europe Market Scanner v5.2.3 — Price × Volume':'Albion Europe Market Scanner v5.2.3 — Cena × Wolumen';
...
function currentSettings(){
  const ids=['languageSelect','itemQuery','tier','enchant','quality','scanLimit','mode','riskProfile','analysisDepth','premium','setupFee','undercut','transport','minProfit','minVolumeDay','excludeSame','portfolioBudget','portfolioReservePct','portfolioMaxItemPct','portfolioMaxRoutePct','portfolioMaxPositions','portfolioMaxUnits','portfolioLiquidityDays','autoPortfolio','autoRefresh','autoStart','useCache'];
  const v={};for(const id of ids){const x=el(id);if(x)v[id]=x.type==='checkbox'?x.checked:x.value;}
  v.markets=selectedMarkets();v.categories=selectedCategories();v._uiSchema=3;return v;
}
...
async function loadSettings(){
  ...
  const v={...rec.value};
  // v5.2.3: simplify result criteria to price + volume only.
  if((+v._uiSchema||1)<3){Object.assign(v,{minProfit:'0',minVolumeDay:'0',_uiSchema:3});}
...
function cfgFromUI(){return{minProfit:+el('minProfit')?.value||0,minVolumeDay:+el('minVolumeDay')?.value||0,excludeSame:!!el('excludeSame')?.checked,tax:el('premium').value==='yes'?4:8,setup:+el('setupFee').value||0,undercut:+el('undercut').value||0,transport:+el('transport').value||0,riskProfile:el('riskProfile').value||'balanced',maxAge:72};}
  ...
  for(const [itemId,rows] of rowsByItem){
    ...
    for(const qRows of byQ.values()){
      ...
      for(const b of buys){for(const d of destinations){
        ...
        const o={schemaVersion:9,key:`${itemId}|${b.quality}|${b.city}|${d.city}|${mode}`,itemId,itemName:itemDisplayName(item),itemNamePl:item.namePl||item.name,itemNameEn:item.nameEn||item.name,quality:+b.quality,source:b.city,dest:d.city,buy,sell:gross,profit,roi,fees,taxFee:tax,setupFee:setup,transportCost:cfg.transport,capitalPerUnit,sourceDate:b.sell_price_min_date,destDate:d._destDate,age:base.age,confidence:base.confidence,components:base.components,warnings:base.warnings,cityMedian,cityDiscount,mode,history:null,calculatedAt:now(),sourceRow:b,destRow:d};
        recalcScore(o,cfg);result.push(o);md.acceptedPairs++;
      }}
    }
  }
  // Keep several route alternatives for each item/quality/model instead of collapsing everything to one route.
  const groups=new Map();for(const o of result){const k=o.itemId+'|'+o.quality+'|'+o.mode;if(!groups.has(k))groups.set(k,[]);groups.get(k).push(o);}
  const out=[];for(const arr of groups.values()){arr.sort((a,b)=>profitScore(b.profit,b.roi)-profitScore(a.profit,a.roi)||(b.profit||0)-(a.profit||0));arr.slice(0,5).forEach((o,i)=>{o.routeRank=i+1;out.push(o);});}
  md.signals=out.length;return out;
}
...
async function enrichOne(o,cfg){
  ...
  o.volume={source:sm?.volumes||null,destination:dm?.volumes||null,sourceRegularity:sm?.regularity??0,destinationRegularity:dm?.regularity??null,effectiveDaily,liquidityScore:liq,sourceLiquidity:srcLiq,destinationLiquidity:dstLiq,trendPct:dm?.trendPct??sm?.trendPct??0,qty,profitPerDayNominal:Math.max(0,o.profit)*dailyModelUnits,profitPerDayModel:Math.max(0,o.profit)*dailyModelUnits,proxy:o.mode==='instant'?'destination sell-history volume is a liquidity proxy, not buy-order depth':null};
...
function renderScanDiagnostics(){
  ...
  const msg=state.lang==='en'?`Selected ${fmt(d.selectedItems||0)} / matching ${fmt(d.matchingItems||0)} • API rows ${fmt(d.apiRows||0)} • item IDs with data ${fmt(d.itemsWithRows||0)} • pairs ${fmt(t.pairs)} • non-positive ${fmt(t.nonPositive)} • positive route candidates ${fmt(t.accepted)} • saved signals ${fmt(state.opportunities.length)} • shown ${fmt(shown)}`:`Wybrano ${fmt(d.selectedItems||0)} z ${fmt(d.matchingItems||0)} pasujących • rekordy API ${fmt(d.apiRows||0)} • itemy z danymi ${fmt(d.itemsWithRows||0)} • pary ${fmt(t.pairs)} • niedodatnie ${fmt(t.nonPositive)} • dodatnie kandydaty tras ${fmt(t.accepted)} • zapisane sygnały ${fmt(state.opportunities.length)} • pokazane ${fmt(shown)}`;
  ...
  if(!shown&&el('emptyState')){
    ...
    else hint+=state.lang==='en'?'Signals exist — press “All positive” or lower the result filters.':'Sygnały istnieją — kliknij „Wszystkie dodatnie” albo zmniejsz filtry wyników.';
...
async function scan(opts={}){
  ...
  const depth=+el('analysisDepth').value||0;if(depth>0&&!state.stop){setProgress(83,state.lang==='en'?`Initial ${fmt(state.opportunities.length)} opportunities. Calculating volume for top price candidates…`:`Wstępnie ${fmt(state.opportunities.length)} okazji. Liczę wolumen dla najlepszych cenowo kandydatów…`);await enrichTop(Math.min(depth,state.opportunities.length));}
...
function filteredResults(){
  ...
  // Only two result criteria: net price profit and historical volume.
  a=a.filter(o=>(o.profit??-Infinity)>=cfg.minProfit);
  if(cfg.minVolumeDay>0)a=a.filter(o=>o.volume&&(o.volume.effectiveDaily||0)>=cfg.minVolumeDay);
  const sort=el('sortBy').value;
  a.sort((x,y)=>sort==='priceVolume'?(y.priceVolumeScore||y.score||0)-(x.priceVolumeScore||x.score||0):sort==='profit'?y.profit-x.profit:sort==='roi'?y.roi-x.roi:sort==='age'?x.age-y.age:sort==='buy'?x.buy-y.buy:sort==='confidence'?(y.confidence||0)-(x.confidence||0):sort==='riskProfit'?(y.riskProfit||0)-(x.riskProfit||0):sort==='liquidity'?(y.volume?.liquidityScore||0)-(x.volume?.liquidityScore||0):sort==='volume'?(y.volume?.effectiveDaily||0)-(x.volume?.effectiveDaily||0):sort==='profitDay'?(y.volume?.profitPerDayNominal||0)-(x.volume?.profitPerDayNominal||0):sort==='exit'?(x.volume?.qty?.exitDaysNormal??Infinity)-(y.volume?.qty?.exitDaysNormal??Infinity):(y.score||0)-(x.score||0));return a;
}
...
function openDetail(o){
  ...
  const pfPos=state.portfolio?.positions?.find(x=>x.opportunityKey===o.key);const pfInfo=pfPos?(state.lang==='en'?`<br><b>Portfolio v5.2:</b> priority #${pfPos.rank}, ${fmt(pfPos.units)} units, capital ${fmt(pfPos.capital)}, model profit ${fmt(pfPos.modelProfit)}.`:`<br><b>Portfel v5.2.3:</b> priorytet #${pfPos.rank}, ${fmt(pfPos.units)} szt., kapitał ${fmt(pfPos.capital)}, zysk modelowy ${fmt(pfPos.modelProfit)}.`):'';
  el('detailNote').innerHTML=state.lang==='en'?`Buy: <b>${fmt(o.buy)}</b> in ${esc(marketLabel(o.source))}. Target: <b>${fmt(o.sell)}</b> in ${esc(marketLabel(o.dest))}. Required capital / unit: <b>${fmt(portfolioCapitalPerUnit(o))}</b>. Confidence-weighted profit: <b>${fmt(o.riskProfit)}</b>. Price × Volume Score: <b>${(o.score||0).toFixed(0)}/100</b>.${hist}${volInfo}${pfInfo}${warnings}<br><span class="muted">Confidence Score is a heuristic signal-quality measure, not a guarantee of execution or available quantity.</span>${breakdown}`:`Kupno: <b>${fmt(o.buy)}</b> w ${esc(marketLabel(o.source))}. Cel: <b>${fmt(o.sell)}</b> w ${esc(marketLabel(o.dest))}. Kapitał wymagany / szt.: <b>${fmt(portfolioCapitalPerUnit(o))}</b>. Zysk ważony pewnością: <b>${fmt(o.riskProfit)}</b>. Price × Volume Score: <b>${(o.score||0).toFixed(0)}/100</b>.${hist}${volInfo}${pfInfo}${warnings}<br><span class="muted">Confidence Score jest heurystyką jakości sygnału, nie gwarancją wykonania transakcji ani dostępnej liczby sztuk.</span>${breakdown}`;
