...
Object.assign(TEXT_PAIRS,{
  ...
  'Handel hurtowy / Mamut':'Bulk trade / Mammoth','Preset: runy / dusze / relikty':'Preset: runes / souls / relics',
...
function attachSimpleVolume(o,cache,failedKeys=new Set()){
  ...
  const buyDay=sm?.volumes?.d7?.avg??(srcRec?.confirmed?0:null),sellDay=o.dest==='Black Market'?null:(dm?.volumes?.d7?.avg??(dstRec?.confirmed?0:null)),tradeDay=Number.isFinite(buyDay)&&Number.isFinite(sellDay)?Math.min(buyDay,sellDay):null,src7=sm?.volumes?.d7?.total??(srcRec?.confirmed?0:null),dst7=o.dest==='Black Market'?null:(dm?.volumes?.d7?.total??(dstRec?.confirmed?0:null)),src30=sm?.volumes?.d30?.total??(srcRec?.confirmed?0:null),dst30=o.dest==='Black Market'?null:(dm?.volumes?.d30?.total??(dstRec?.confirmed?0:null)),trade7=Number.isFinite(src7)&&Number.isFinite(dst7)?Math.min(src7,dst7):null,trade30=Number.isFinite(src30)&&Number.isFinite(dst30)?Math.min(src30,dst30):null;
  o.volume={source:sm?.volumes||null,destination:dm?.volumes||null,buyDaily:buyDay,sellDaily:sellDay,effectiveDaily:tradeDay,trade7,trade30,profitVolumeDaily:0,sourceStatus:srcState,destinationStatus:dstState};
...
function bulkPlanFor(o,cfg){
  const daily=Number.isFinite(o.volume?.effectiveDaily)?Math.max(0,o.volume.effectiveDaily):0,share=clamp(cfg.bulkVolumeShare,1,100)/100;
  const volumeCap=daily>0?Math.max(0,Math.floor(daily*cfg.bulkDays*share)):0;
  const routeCap=Math.max(0,Math.min(volumeCap,Math.max(0,Math.floor(cfg.bulkMaxQty||0))));
  ...
  const sellRate=daily*share;
  return{...totals,unitCapital:qty>0?totals.capital/qty:approxUnitCapital,daysAtVolume:sellRate>0?qty/sellRate:null,daysAtFullVolume:daily>0?qty/daily:null,volumeCap,budgetCap,volumeShare:share};
}
