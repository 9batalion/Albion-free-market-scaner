...
function attachSimpleVolume(o,cache,failedKeys=new Set()){
  const srcKey=volumeHistoryKey(o.itemId,o.source,o.quality),dstKey=o.dest==='Black Market'?null:volumeHistoryKey(o.itemId,o.dest,o.quality),srcRec=cache.get(srcKey),dstRec=dstKey?cache.get(dstKey):null;
  const srcState=volumeRecordState(srcRec,failedKeys.has(srcKey)),dstState=o.dest==='Black Market'?'unavailable':volumeRecordState(dstRec,failedKeys.has(dstKey));
  const sm=srcRec?historyMetrics(srcRec.data||[]):null,dm=dstRec?historyMetrics(dstRec.data||[]):null;
  ...
  const tradeDay=Number.isFinite(buyDay)&&Number.isFinite(sellDay)?Math.min(buyDay,sellDay):null;
...
function filteredResults(){
  ...
  a.sort((x,y)=>sort==='bulkProfit'?(y.bulk?.totalProfit||0)-(x.bulk?.totalProfit||0):sort==='profit'?y.profit-x.profit:sort==='volume'?(y.volume?.effectiveDaily??-1)-(x.volume?.effectiveDaily??-1):sort==='buy'?x.buy-y.buy:sort==='sell'?y.sell-x.sell:(y.profitVolumeDaily||0)-(x.profitVolumeDaily||0));return a;
}
...
function render(){
  ...
  el('resultsBody').innerHTML=pageRows.map((o,i)=>{const v=o.volume||{},v7=Number.isFinite(v.trade7)?fmt(v.trade7):'—',v30=Number.isFinite(v.trade30)?fmt(v.trade30):'—';const bulk=o.bulk||bulkPlanFor(o,cfgFromUI());return `<tr><td><button class="star ${state.watched.has(o.itemId)?'on':''}" data-watch="${esc(o.itemId)}">★</button></td><td><div class="itemcell"><img loading="lazy" src="${itemIcon(o.itemId,o.quality)}" alt=""><div><div class="itemname">${esc(displayOpportunityName(o))}</div><div class="itemid">${esc(o.itemId)} • ${qualityName[o.quality]||o.quality}</div></div></div></td><td><span class="tag info">${esc(simpleModeLabel(o))}</span></td><td class="route"><b>${esc(marketLabel(o.source))}</b></td><td>${fmt(o.buy)}</td><td class="route"><b>${esc(marketLabel(o.dest))}</b></td><td>${fmt(o.sell)}</td><td class="profit">+${fmt(o.profit)}</td><td><b>${volumeFmt(v.effectiveDaily,(v.sourceStatus==='error'||v.destinationStatus==='error')?'error':(v.sourceStatus?.includes('cache')||v.destinationStatus?.includes('cache'))?'stale-cache':'fresh')}</b></td><td><b>${fmt(bulk.qty)}</b></td><td>${fmt(bulk.capital)}</td><td class="profit">+${fmt(bulk.totalProfit)}</td><td class="profit">${fmt(o.profitVolumeDaily||0)}</td><td><button class="btn" data-detail="${i}">${tr('Szczegóły','Details')}</button></td></tr>`;}).join('');
