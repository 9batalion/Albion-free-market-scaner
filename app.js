...
// v5.4 — hard anti-anomaly guard + Market Reality layer. We still keep the highest REAL market price.
...
function historyReferencePrice(m){
  const refs=[m?.prices?.d7?.vwap,m?.prices?.d7?.median,m?.prices?.d14?.vwap,m?.prices?.d14?.median,m?.prices?.d30?.vwap,m?.vwap,m?.median].map(Number).filter(v=>Number.isFinite(v)&&v>0);
...
function marketRealityAssessment(o,sm,dm){
  ...
  let score=Math.round(.35*freshness+.25*liquidity+.20*regularity+.20*plausibility);if(o.dest==='Black Market'&&!dm)score=Math.max(0,score-8);
  ...
  return{score,grade,label,buyAge,sellAge,worstAge,buyRef,sellRef,buyRatio,sellRatio,freshness,liquidity,regularity,plausibility,repeatableProfit,repeatableMargin};
}
...
function applyLanguage(lang,persist=true){
  ...
  document.title=state.lang==='en'?'Albion Europe Market Scanner v5.3 — Bulk Trade':'Albion Europe Market Scanner v5.3 — Hurt / Mamut';
...
function historyMetrics(data,windowDays=30){
  ...
  const priceWin=n=>{const a=complete.slice(0,n).filter(x=>Number.isFinite(x.price)&&x.price>0),prices=a.map(x=>x.price),weighted=a.filter(x=>x.volume>0),weight=weighted.reduce((s,x)=>s+x.volume,0),vwap=weight>0?weighted.reduce((s,x)=>s+x.price*x.volume,0)/weight:mean(prices);return{days:n,points:a.length,median:prices.length?median(prices):null,avg:prices.length?mean(prices):null,vwap:Number.isFinite(vwap)?vwap:null};};
...
function attachSimpleVolume(o,cache,failedKeys=new Set()){
  ...
  const destinationHistoryOutlier=o.dest!=='Black Market'&&historyPriceOutlier(o.sell,dm,'high'),sourceHistoryOutlier=historyPriceOutlier(o.buy,sm,'low'),historyOutlier=destinationHistoryOutlier||sourceHistoryOutlier;
  o.priceGuard={...(o.priceReference||{}),historyMedian:dm?.median??null,historyReference:historyReferencePrice(dm),sourceHistoryMedian:sm?.median??null,sourceHistoryReference:historyReferencePrice(sm),historyOutlier,destinationHistoryOutlier,sourceHistoryOutlier};o.invalidPrice=historyOutlier;o.volume.profitVolumeDaily=historyOutlier?0:(Number.isFinite(tradeDay)?Math.max(0,o.profit)*tradeDay:0);o.profitVolumeDaily=o.volume.profitVolumeDaily;o.history={source:sm,destination:dm,points:srcRec?.data||[],basis:'AODP sell history; robust 7/14/30d price references; not current order-book depth'};o.reality=marketRealityAssessment(o,sm,dm);return o;
}
...
function bulkPlanFor(o,cfg){
  const daily=Number.isFinite(o.volume?.effectiveDaily)?Math.max(0,o.volume.effectiveDaily):0;
  const unitCapital=Math.max(1,(o.buy||0)+(o.transportCost||0)+(o.mode==='relist'?(o.setupFee||0):0));
  const volumeCap=daily>0?Math.max(1,Math.floor(daily*cfg.bulkDays*(cfg.bulkVolumeShare/100))):0;
  const budgetCap=cfg.bulkBudget>0?Math.floor(cfg.bulkBudget/unitCapital):0;
  const qty=Math.max(0,Math.min(volumeCap,budgetCap,cfg.bulkMaxQty));
  return{qty,unitCapital,capital:qty*unitCapital,buyCost:qty*(o.buy||0),revenue:qty*(o.sell||0),totalProfit:qty*(o.profit||0),daysAtVolume:daily>0?qty/daily:null,volumeCap,budgetCap};
}
...
function render(){
  ...
  el('resultsBody').innerHTML=pageRows.map((o,i)=>{const v=o.volume||{},bulk=o.bulk||bulkPlanFor(o,cfgFromUI()),r=o.reality||{},gc=r.grade==='A'?'good':r.grade==='B'?'info':r.grade==='C'?'warn':'bad';return `<tr><td><button class="star ${state.watched.has(o.itemId)?'on':''}" data-watch="${esc(o.itemId)}">★</button></td><td><div class="itemcell"><img loading="lazy" src="${itemIcon(o.itemId,o.quality)}" alt=""><div><div class="itemname">${esc(displayOpportunityName(o))}</div><div class="itemid">${esc(o.itemId)} • ${qualityName[o.quality]||o.quality}</div></div></div></td><td><span class="tag info">${esc(simpleModeLabel(o))}</span></td><td class="route"><b>${esc(marketLabel(o.source))}</b>${o.bestSource?` <span class="tag good">${tr('najtańszy','cheapest')}</span>`:''}</td><td>${fmt(o.buy)}</td><td class="route"><b>${esc(marketLabel(o.dest))}</b>${o.bestDestination?` <span class="tag good">${tr('#1 sprzedaż','#1 sell')}</span>`:''}</td><td>${fmt(o.sell)}</td><td class="profit">+${fmt(o.profit)}</td><td><span class="tag ${gc}">${r.grade||'—'} ${esc(r.label||'')}</span><div class="itemid">${Number.isFinite(r.score)?r.score+'/100':'—'} • ${ageText(r.worstAge)}</div></td><td>${Number.isFinite(r.sellRef)?fmt(r.sellRef):'—'}</td><td><b>${volumeFmt(v.effectiveDaily,(v.sourceStatus==='error'||v.destinationStatus==='error')?'error':(v.sourceStatus?.includes('cache')||v.destinationStatus?.includes('cache'))?'stale-cache':'fresh')}</b></td><td><b>${fmt(bulk.qty)}</b></td><td>${fmt(bulk.capital)}</td><td class="profit">+${fmt(bulk.totalProfit)}</td><td class="profit">${fmt(o.profitVolumeDaily||0)}</td><td><button class="btn" data-detail="${i}">${tr('Szczegóły','Details')}</button></td></tr>`;}).join('');
...
function exportCsv(){const a=filteredResults();if(!a.length)return;const cols=['item_id','name','quality','mode','buy_market','buy_price','sell_market','sell_price','net_profit_unit','market_reality_score','market_reality_grade','buy_ref_7d','sell_ref_7d','buy_price_age_h','sell_price_age_h','historical_profit_unit','best_source','best_destination','buy_volume_day','sell_volume_day','trade_volume_day','trade_volume_7d','trade_volume_30d','profit_x_volume_day','planned_units','planned_capital','estimated_trip_profit'],lines=[cols.join(';'),...a.map(o=>[o.itemId,displayOpportunityName(o),qualityName[o.quality],simpleModeLabel(o),marketLabel(o.source),Math.round(o.buy),marketLabel(o.dest),Math.round(o.sell),Math.round(o.profit),o.reality?.score??'',o.reality?.grade??'',Number.isFinite(o.reality?.buyRef)?Math.round(o.reality.buyRef):'',Number.isFinite(o.reality?.sellRef)?Math.round(o.reality.sellRef):'',Number.isFinite(o.reality?.buyAge)?o.reality.buyAge.toFixed(2):'',Number.isFinite(o.reality?.sellAge)?o.reality.sellAge.toFixed(2):'',Number.isFinite(o.reality?.repeatableProfit)?Math.round(o.reality.repeatableProfit):'',o.bestSource?'yes':'',o.bestDestination?'yes':'',Number.isFinite(o.volume?.buyDaily)?o.volume.buyDaily.toFixed(2):'',Number.isFinite(o.volume?.sellDaily)?o.volume.sellDaily.toFixed(2):'',Number.isFinite(o.volume?.effectiveDaily)?o.volume.effectiveDaily.toFixed(2):'',Number.isFinite(o.volume?.trade7)?Math.round(o.volume.trade7):'',Number.isFinite(o.volume?.trade30)?Math.round(o.volume.trade30):'',Math.round(o.profitVolumeDaily||0),o.bulk?.qty||0,Math.round(o.bulk?.capital||0),Math.round(o.bulk?.totalProfit||0)].map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(';'))],blob=new Blob(['\ufeff'+lines.join('\n')],{type:'text/csv;charset=utf-8'}),u=URL.createObjectURL(blob),ael=document.createElement('a');ael.href=u;ael.download='albion_europe_bulk_trade_v5_4.csv';ael.click();setTimeout(()=>URL.revokeObjectURL(u),1000);}

async function init(){renderMarkets();renderCategories(CATEGORY_LEAVES);bind();try{await openLocalDB();setDbStatus(tr('gotowa','ready'));const w=await dbGetAll('watchlist');state.watched=new Set(w.map(x=>x.itemId));await loadSettings();await updateDbInfo();setProgress(3,tr('Ładowanie bazy przedmiotów…','Loading item database…'));await loadItems();await loadCachedOpportunities();setProgress(0,state.opportunities.length?tr('Gotowy — pokazano ostatni cache.','Ready — showing latest cache.'):tr('Gotowy — łączenie z API…','Ready — connecting to API…'));await pingApi();scheduleAuto();if(el('autoStart').value==='yes'&&navigator.onLine)setTimeout(()=>scan({auto:true}).catch(()=>{}),700);}catch(e){setDbStatus(tr('błąd','error'),false);el('dbStatus').textContent=tr('błąd','error');setProgress(0,tr('Błąd inicjalizacji: ','Initialization error: ')+(e.message||e));}
