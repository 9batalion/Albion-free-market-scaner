...
function applyLanguage(lang,persist=true){
  ...
  document.title=state.lang==='en'?'Albion Europe Market Scanner v5.4.3 — Target Volume':'Albion Europe Market Scanner v5.4.3 — Wolumen celu';
...
function render(){
  ...
  el('resultsBody').innerHTML=pageRows.map((o,i)=>{const v=o.volume||{},bulk=o.bulk||bulkPlanFor(o,cfgFromUI()),r=o.reality||{},gc=r.grade==='A'?'good':r.grade==='B'?'info':r.grade==='C'?'warn':'bad';return `<tr><td><button class="star ${state.watched.has(o.itemId)?'on':''}" data-watch="${esc(o.itemId)}">★</button></td><td><div class="itemcell"><img loading="lazy" src="${itemIcon(o.itemId,o.quality)}" alt=""><div><div class="itemname">${esc(displayOpportunityName(o))}</div><div class="itemid">${esc(o.itemId)} • ${qualityName[o.quality]||o.quality}</div></div></div></td><td><span class="tag info">${esc(simpleModeLabel(o))}</span></td><td class="route"><b>${esc(marketLabel(o.source))}</b>${o.bestSource?` <span class="tag good">${tr('najtańszy','cheapest')}</span>`:''}${o.priceGuard?.sourceWarning?` <span class="tag warn">${tr('sprawdź cenę','verify price')}</span>`:''}</td><td>${fmt(o.buy)}</td><td class="route"><b>${esc(marketLabel(o.dest))}</b>${o.bestDestination?` <span class="tag good">${tr('#1 sprzedaż','#1 sell')}</span>`:''}</td><td>${fmt(o.sell)}</td><td class="profit">+${fmt(o.profit)}</td><td><span class="tag ${gc}">${r.grade||'—'} ${esc(r.label||'')}</span><div class="itemid">${Number.isFinite(r.score)?r.score+'/100':'—'} • ${tr('dane','data')} ${Number.isFinite(r.dataCompleteness)?r.dataCompleteness+'%':'—'} • ${ageText(r.worstAge)}</div></td><td>${Number.isFinite(r.sellRef)?fmt(r.sellRef):'—'}</td><td><b>${volumeFmt(v.sourceSafeDaily,v.sourceStatus)}</b><div class="itemid">${Number.isFinite(v.buyDaily)?tr('śr. ','avg. ')+volumeFmt(v.buyDaily,v.sourceStatus):''}</div></td><td><b>${volumeFmt(v.destinationSafeDaily,v.destinationStatus)}</b><div class="itemid">${Number.isFinite(v.sellDaily)?tr('śr. ','avg. ')+volumeFmt(v.sellDaily,v.destinationStatus):''}</div></td><td><b>${fmt(bulk.qty)}</b><div class="itemid">${tr('cel','target')} ${Math.round((bulk.volumeShare||0)*100)}%</div></td><td>${fmt(bulk.capital)}</td><td class="profit">+${fmt(bulk.totalProfit)}</td><td class="profit">${fmt(o.profitVolumeDaily||0)}</td><td><button class="btn" data-detail="${i}">${tr('Szczegóły','Details')}</button></td></tr>`;}).join('');
...
async function loadCachedOpportunities(){
  ...
  // v5.4.3 accepts only the current opportunity schema so stale planning models cannot reappear.
  ops=ops.filter(x=>x&&x.schemaVersion===16);
...
async function exportDB(){
  ...
  a.href=u;a.download=`albion-local-db-v5_4_3-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);
}
...
async function init(){
  ...
  if('serviceWorker' in navigator)navigator.serviceWorker.register('./service-worker.js?v=5.4.3').catch(()=>{});
...
function exportCsv(){
  ...
  const blob=new Blob(['\ufeff'+lines.join('\n')],{type:'text/csv;charset=utf-8'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='albion_europe_bulk_trade_v5_4_3.csv';a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);
}
init();
})();
