...
const DB_VERSION=15;
const ITEM_CATALOG_SCHEMA=2;
const DB_STORES=['items','prices','history','watchlist','settings','opportunities','scan_runs'];
const el = id => document.getElementById(id);
const state = {items:[],itemById:new Map(),raw:[],opportunities:[],t3ResourceCoverage:[],t3FailedPriceIds:new Set(),stop:false,watched:new Set(),history:new Map(),scanId:0,watchOnly:false,lastApiOk:false,lang:'pl',scanDiagnostics:null,page:1,pageSize:100,volumeFailures:new Set(),volumeStats:{jobs:0,errors:0},manualScanUnlocked:false,sortDirection:'desc'};
let db=null,settingsTimer=null,autoTimer=null,memoryAutoId=0;
const memoryStores=new Map(['items','prices','history','watchlist','settings','opportunities','scan_runs'].map(name=>[name,new Map()]));
...
// Fixed luxury payouts do not have an AODP timestamp; only the purchase quote ages.
const routeQuoteAgeHours = o => o?.fixedPayout?ageHours(o?.sourceDate):Math.max(ageHours(o?.sourceDate),ageHours(o?.destDate));
...
const SORT_KEYS=new Set(['freshness','item','mode','source','buyTime','dest','sellTime','profit','reality','sellRef','sourceVolume','volume','qty','capital','bulkProfit','profitVolume']);
function defaultSortDirection(key){return ['item','mode','source','dest'].includes(key)?'asc':'desc';}
function compareOpportunityBySort(x,y,key){
  ...
  if(key==='buyTime')return num(parseAodpTime(y.sourceDate),parseAodpTime(x.sourceDate));
  if(key==='sellTime')return num(parseAodpTime(y.fixedPayout?y.sourceDate:y.destDate),parseAodpTime(x.fixedPayout?x.sourceDate:x.destDate));
...
function quoteTimestampText(d){
  ...
  return `${local} ${tr('lokalnie','local')} • ${utc} UTC • ${ageText(ageHours(d))} ${tr('temu','ago')}`;
}
const marketLabel = api => api==='Any Royal City'?tr('Dowolne Królewskie Miasto','Any Royal City'):(MARKETS.find(m=>m.api===api)?.label || api);
const saleTimestampText = o => o?.fixedPayout?tr('stała cena w grze • bez odczytu AODP','fixed in-game payout • no AODP quote'):quoteTimestampText(o?.destDate);
const destinationVolumeText = o => o?.fixedPayout?tr('stały skup','fixed buyback'):volumeFmt(o?.volume?.destinationSafeDaily,o?.volume?.destinationStatus);
...
// v5.4.1 — asymmetric price guard + robust history + data completeness.
// High destination anomalies can be rejected. Extremely low source prices are preserved as opportunities,
// but marked for manual verification because a fresh mistaken listing can be genuinely buyable.
function marketPriceGuard(rows,field){
  ...
  // Wide guard: normal city spreads must survive. It is intentionally much stricter only for extreme highs.
...
function historyReferencePrice(m){
  // Median first: the anti-anomaly reference must not be dragged upward by one huge, high-volume day.
  // robustVwap is winsorized around the daily median/MAD and is a secondary reference.
  const refs=[m?.prices?.d7?.median,m?.prices?.d7?.robustVwap,m?.prices?.d14?.median,m?.prices?.d14?.robustVwap,m?.prices?.d30?.median,m?.prices?.d30?.robustVwap,m?.median,m?.vwap]
    .map(Number).filter(v=>Number.isFinite(v)&&v>0);
  return refs.length?refs[0]:null;
}
function historyRatioLimit(m){
  const daily=+m?.volumes?.d7?.avg||0,regularity=+m?.regularity||0;
  let cap=daily>=500?3.5:daily>=100?4:daily>=20?5:daily>=5?7:10;
  if(regularity<35)cap*=1.25;
  return cap;
}
function historyPriceOutlier(currentPrice,histMetrics,direction='high'){
  const ref=historyReferencePrice(histMetrics);if(!(currentPrice>0&&ref>0))return false;const cap=historyRatioLimit(histMetrics);
  return direction==='low'?currentPrice<ref/cap:currentPrice>ref*cap;
}
function freshnessScoreFromAge(h){if(!Number.isFinite(h))return 15;if(h<=1)return 100;if(h<=3)return 92;if(h<=6)return 82;if(h<=12)return 68;if(h<=24)return 52;if(h<=48)return 32;if(h<=96)return 18;return 8;}
function ratioPlausibilityScore(r){if(!(r>0&&Number.isFinite(r)))return 65;return clamp(100-Math.abs(Math.log(r))*42,5,100);}
function historyCompletenessPoints(m){
  if(!m||!historyReferencePrice(m))return 0;
  const p7=+m?.prices?.d7?.points||0;
  if(p7>=5)return 30;if(p7>=3)return 26;if(p7>=1)return 18;return 12;
}
function marketRealityAssessment(o,sm,dm){
  if(o.fixedPayout){
    const buyRef=historyReferencePrice(sm),buyRatio=buyRef?o.buy/buyRef:null,buyAge=ageHours(o.sourceDate),freshness=freshnessScoreFromAge(buyAge),sourceDaily=Number.isFinite(o.volume?.sourceSafeDaily)?o.volume.sourceSafeDaily:null,liquidity=Number.isFinite(sourceDaily)?clamp(25*Math.log10(1+Math.max(0,sourceDaily)),0,100):35,regularity=Number.isFinite(+sm?.regularity)?+sm.regularity:45,plausibility=ratioPlausibilityScore(buyRatio),rawScore=Math.round(.45*freshness+.25*liquidity+.15*regularity+.15*plausibility),completeness=clamp(Math.round((Number.isFinite(buyAge)?35:0)+historyCompletenessPoints(sm)+35),0,100),scoreCap=completeness<70?67:completeness<90?81:100,score=Math.min(rawScore,scoreCap),grade=score>=82?'A':score>=68?'B':score>=52?'C':'D',label=grade==='A'?tr('mocna','strong'):grade==='B'?tr('dobra','good'):grade==='C'?tr('ostrożnie','caution'):tr('sprawdź','verify');
    return{score,rawScore,scoreCap,grade,label,dataCompleteness:completeness,buyAge,sellAge:0,worstAge:buyAge,buyRef,sellRef:o.sell,buyRatio,sellRatio:1,freshness,liquidity,regularity,plausibility,repeatableProfit:null,repeatableMargin:null};
  }
  const buyRef=historyReferencePrice(sm),sellRef=historyReferencePrice(dm),buyRatio=buyRef?o.buy/buyRef:null,sellRatio=sellRef?o.sell/sellRef:null;
  const buyAge=ageHours(o.sourceDate),sellAge=ageHours(o.destDate),worstAge=Math.max(buyAge,sellAge),tradeDay=Number.isFinite(o.volume?.effectiveDaily)?o.volume.effectiveDaily:null;
  const freshness=freshnessScoreFromAge(worstAge),liquidity=Number.isFinite(tradeDay)?clamp(25*Math.log10(1+Math.max(0,tradeDay)),0,100):35;
  const regs=[sm?.regularity,dm?.regularity].map(Number).filter(Number.isFinite),regularity=regs.length?Math.min(...regs):45,prs=[buyRatio,sellRatio].filter(v=>v>0&&Number.isFinite(v)).map(ratioPlausibilityScore),plausibility=prs.length?mean(prs):65;
  const rawScore=Math.round(.35*freshness+.25*liquidity+.20*regularity+.20*plausibility);
  let completeness=(Number.isFinite(buyAge)?20:0)+(Number.isFinite(sellAge)?20:0)+historyCompletenessPoints(sm);
  // Black Market has a different demand mechanism and no comparable sell-history depth for its current buy order.
  // Give partial credit for a known limitation, but never let it receive grade A from incomplete comparable data.
  completeness+=o.dest==='Black Market'?15:historyCompletenessPoints(dm);
  completeness=clamp(Math.round(completeness),0,100);
  const scoreCap=completeness<50?51:completeness<75?67:completeness<90?81:100;
  const score=Math.min(rawScore,scoreCap);
  const grade=score>=82?'A':score>=68?'B':score>=52?'C':'D',label=grade==='A'?tr('mocna','strong'):grade==='B'?tr('dobra','good'):grade==='C'?tr('ostrożnie','caution'):tr('sprawdź','verify');
  let repeatableProfit=null,repeatableMargin=null;if(o.mode==='relist'&&buyRef&&sellRef){const ur=o.rawDestinationPrice>0?o.sell/o.rawDestinationPrice:1,hg=Math.max(1,Math.floor(sellRef*ur)),tax=Math.ceil(hg*(+o.taxRatePct||0)/100),setup=Math.ceil(hg*(+o.setupRatePct||0)/100);repeatableProfit=hg-tax-setup-buyRef-(+o.transportCost||0);repeatableMargin=buyRef>0?repeatableProfit/buyRef*100:null;}
  return{score,rawScore,scoreCap,grade,label,dataCompleteness:completeness,buyAge,sellAge,worstAge,buyRef,sellRef,buyRatio,sellRatio,freshness,liquidity,regularity,plausibility,repeatableProfit,repeatableMargin};
}
function markRouteRanks(ops){const groups=new Map();for(const o of ops||[]){const k=`${o.itemId}|${o.quality}|${o.mode}`;if(!groups.has(k))groups.set(k,[]);groups.get(k).push(o);}for(const arr of groups.values()){const bestSell=Math.max(...arr.map(x=>+x.sell||0)),bestBuy=Math.min(...arr.map(x=>+x.buy||Infinity));for(const o of arr){o.bestDestination=(+o.sell||0)===bestSell;o.bestSource=(+o.buy||Infinity)===bestBuy;}}}
...
const TEXT_PAIRS = {
  ...
  'Poziom przedmiotu (Tier)':'Item tier','Wszystkie poziomy T2–T8':'All tiers T2–T8','Poziom zaczarowania':'Enchantment level','Wszystkie poziomy zaczarowania':'All enchantment levels','Brak zaczarowania (.0)':'No enchantment (.0)','Poziom 2 (T2)':'Tier 2 (T2)','Poziom 3 (T3)':'Tier 3 (T3)','Poziom 4 (T4)':'Tier 4 (T4)','Poziom 5 (T5)':'Tier 5 (T5)','Poziom 6 (T6)':'Tier 6 (T6)','Poziom 7 (T7)':'Tier 7 (T7)','Poziom 8 (T8)':'Tier 8 (T8)','Zaczarowanie 1 (.1)':'Enchantment 1 (.1)','Zaczarowanie 2 (.2)':'Enchantment 2 (.2)','Zaczarowanie 3 (.3)':'Enchantment 3 (.3)','Zaczarowanie 4 (.4)':'Enchantment 4 (.4)','Wszystkie T2–T8':'All T2–T8','Przy pustym wyszukiwaniu skaner wybiera handlowalne T2–T8. Duży limit = więcej zapytań i dłuższy skan.':'With an empty search, the scanner selects tradeable T2–T8 items. A higher limit means more requests and a longer scan.','Przy ustawieniu „Wszystkie T2–T8”, wszystkich kategoriach i limicie 0 skanowany jest cały katalog handlowalnych przedmiotów widoczny u góry. Zapytania są automatycznie dzielone na partie AODP; pełny skan wszystkich jakości może potrwać kilkanaście minut.':'With “All T2–T8”, all categories and limit 0, the complete tradeable item catalog shown above is scanned. Requests are automatically split into AODP batches; a full scan of all qualities may take several minutes.',
  'Tier oznacza poziom przedmiotu, np. T4 to poziom 4. Zaczarowanie to liczba po kropce: .0 oznacza brak zaczarowania, a .1–.4 kolejne poziomy. Przy wszystkich poziomach, wszystkich kategoriach i limicie 0 skanowany jest cały katalog handlowalnych przedmiotów. Pełny skan może potrwać kilkanaście minut.':'Tier is the item level, e.g. T4 means tier 4. Enchantment is the number after the dot: .0 means no enchantment, while .1–.4 are successive enchantment levels. With all tiers, all categories and limit 0, the complete tradeable catalog is scanned. A full scan may take several minutes.',
  ...
  'Wolumen i płynność':'Volume & liquidity','Min. wolumen sprzedaży celu / dzień':'Min. destination sales volume / day','Min. wolumen 7 dni':'Min. 7-day volume','Min. Liquidity Score':'Min. Liquidity Score','Maks. czas wyjścia (dni)':'Max exit time (days)','Tylko po analizie wolumenu':'Require volume analysis','Śr. wolumen':'Avg. volume','est. szt. / dzień':'est. units / day','Śr. płynność':'Avg. liquidity','Wolumen/d':'Volume/day','Wyjście':'Exit','Vol/d':'Vol/day','Sprzedaż cel / dzień':'Destination sales / day','est. płynność rynku':'estimated market liquidity','Wolumen pochodzi z historycznych sell orders AODP. Dla natychmiastowej sprzedaży wolumen rynku docelowego jest wskaźnikiem płynności rynku, a nie głębokością aktualnego buy orderu.':'Volume comes from AODP historical sell orders. For instant sales, destination volume is a market-liquidity proxy, not the depth of the current buy order.',
...
Object.assign(TEXT_PAIRS,{
  'Albion Europe Market Scanner v5.2.5 — Cena i Wolumen':'Albion Europe Market Scanner v5.2.5 — Price & Volume',
  ...
  'Największy wolumen celu':'Highest destination volume','Najlepszy obrót zysku':'Best profit throughput','zysk × wolumen / dzień':'profit × volume / day','Rekordy z wolumenem':'Routes with volume','okazji z historią':'routes with history',
  ...
  'Sortuj: Zysk × wolumen':'Sort: Profit × volume','Zysk / szt.':'Profit / unit','Zysk top / szt.':'Top-quote profit / unit','top quote / szt.':'top quote / unit','Najniższa cena zakupu':'Lowest buy price','Najwyższa cena sprzedaży':'Highest sell price','Sortuj: Najnowszy odczyt ceny kupna':'Sort: Newest buy-price observation','Sortuj: Najnowszy odczyt ceny sprzedaży':'Sort: Newest sell-price observation',
  ...
  '← Poprzednia':'← Previous','Następna →':'Next →','50 / strona':'50 / page','100 / strona':'100 / page','250 / strona':'250 / page',
  ...
  'Plan sztuk = minimum z limitu budżetu, wybranego udziału wolumenu i maksymalnej liczby sztuk. Dla mamuta jest to lista kandydatów do sprawdzenia w grze przed zakupem.':'Planned units = the minimum of budget cap, selected share of volume, and max units. For a mammoth run, treat it as a shortlist to verify in-game before buying.','Skanuj pełny zakres cen i wolumenu':'Scan full price and volume range','Pełny skan respektuje limity AODP i pokazuje postęp liczby przetworzonych przedmiotów. Nie ogranicza się do 500 pozycji.':'Full scan respects AODP limits and shows item processing progress. It is not limited to 500 items.','Domyślnie skanowane są wszystkie pasujące handlowalne T2–T8. Wartość 0 oznacza brak limitu. Zapytania są automatycznie dzielone na partie zgodne z limitem długości URL AODP; pełny skan może potrwać kilkanaście minut.':'By default all matching tradeable T2–T8 items are scanned. 0 means no item limit. Requests are automatically split to fit the AODP URL limit; a full scan may take several minutes.',
  ...
  'Kliknij nagłówek kolumny, aby ją posortować. Drugie kliknięcie odwraca kolejność.':'Click a column header to sort by it. A second click reverses the order.','Kliknij nagłówek kolumny, aby ją posortować. Drugie kliknięcie odwraca kolejność. W kolumnach „Cena kupna” i „Cena sprzedaży” sortowany jest czas odczytu, a nie wysokość ceny.':'Click a column header to sort by it. A second click reverses the order. The Buy price and Sell price columns sort by observation time, not by price value.','Sortuj: Nazwa przedmiotu':'Sort: Item name','Sortuj: Tryb':'Sort: Mode','Sortuj: Miasto zakupu':'Sort: Buy city','Sortuj: Miasto sprzedaży':'Sort: Sell city','Sortuj: Cena referencyjna 7d':'Sort: 7d reference price','Sortuj: Wolumen źródła':'Sort: Source volume','Sortuj: Sztuki do zabrania':'Sort: Units to take','Sortuj: Kapitał':'Sort: Capital',
...
function applyLanguage(lang,persist=true){
  ...
  document.title=state.lang==='en'?'Albion Europe Market Scanner v5.5.6 — Trading and Refining':'Albion Europe Market Scanner v5.5.6 — Handel i rafinacja';
...
const CATEGORY_GROUPS = [
  ...
  {id:'special',pl:'Pozostałe grupy',en:'Other groups',children:[['bulkMaterials','Runy, dusze, relikty i esencje','Runes, souls, relics & essences'],['artifacts','Artefakty','Artifacts'],['journals','Dzienniki','Journals'],['fishing','Ryby i wędkarstwo','Fish & fishing'],['farming','Rolnictwo','Farming'],['luxury','Dobra luksusowe','Luxury goods'],['other','Inne','Other']]}
];
...
const SMALL_GOODS_CATEGORIES = new Set(['bulkMaterials','rawResources','refinedResources','food','potions','farming','fishing','luxury']);
...
const LUXURY_GOOD_ID=/^TREASURE_(?:KNOWLEDGE|SILVERWARE|DECORATIVE|CEREMONIAL|TRIBAL|RITUAL|AVALON)_RARITY[1-3]$/;
const isLuxuryGoodId=id=>LUXURY_GOOD_ID.test(String(id).split('@')[0]);
const LUXURY_GOOD_FALLBACKS=[['TREASURE_KNOWLEDGE_RARITY1','Corrupted Scroll'],['TREASURE_KNOWLEDGE_RARITY2','Thin Booklet'],['TREASURE_KNOWLEDGE_RARITY3','Ancient Tome'],['TREASURE_SILVERWARE_RARITY1','Silver Cup'],['TREASURE_SILVERWARE_RARITY2','Silver Mirror'],['TREASURE_SILVERWARE_RARITY3','Silver Candelabrum'],['TREASURE_DECORATIVE_RARITY1','Simple Stone Toy'],['TREASURE_DECORATIVE_RARITY2','Stone Mask'],['TREASURE_DECORATIVE_RARITY3','Stone Idol'],['TREASURE_CEREMONIAL_RARITY1','Ornamental Scepter'],['TREASURE_CEREMONIAL_RARITY2','Globus Cruciger'],['TREASURE_CEREMONIAL_RARITY3','Golden Crown'],['TREASURE_TRIBAL_RARITY1','Feather Talisman'],['TREASURE_TRIBAL_RARITY2','Dreamcatcher'],['TREASURE_TRIBAL_RARITY3','Holy Fetish'],['TREASURE_RITUAL_RARITY1','Ritual Candles'],['TREASURE_RITUAL_RARITY2','Blood Chalice'],['TREASURE_RITUAL_RARITY3','Sacrificial Dagger'],['TREASURE_AVALON_RARITY1','Golden Frame'],['TREASURE_AVALON_RARITY2','Golden Gyroscope'],['TREASURE_AVALON_RARITY3','Golden Sextant']];
const LUXURY_FIXED_RULES={
  KNOWLEDGE:{destination:'Martlock',payouts:[1000,5000,25000]},
  SILVERWARE:{destination:'Lymhurst',payouts:[1000,5000,25000]},
  DECORATIVE:{destination:'Fort Sterling',payouts:[1000,5000,25000]},
  CEREMONIAL:{destination:'Thetford',payouts:[1000,5000,25000]},
  TRIBAL:{destination:'Bridgewatch',payouts:[1000,5000,25000]},
  RITUAL:{destination:'Caerleon',payouts:[1000,5000,25000]},
  AVALON:{destination:'Any Royal City',payouts:[10000,50000,250000],anyRoyal:true}
};
function luxuryFixedRule(id){const m=/^TREASURE_([A-Z]+)_RARITY([1-3])$/.exec(String(id).split('@')[0]);if(!m)return null;const family=LUXURY_FIXED_RULES[m[1]];return family?{...family,family:m[1],rarity:+m[2],payout:family.payouts[+m[2]-1]}:null;}
function withLuxuryFallbacks(items){const m=new Map((items||[]).map(x=>[x.id,x]));for(const [id,name] of LUXURY_GOOD_FALLBACKS)if(!m.has(id))m.set(id,{id,name,namePl:name,nameEn:name,index:null});return[...m.values()];}
function categoryForItem(id){
  const base=String(id).split('@')[0];
  if(isLuxuryGoodId(base))return'luxury';
  if(/_GATHERER_/.test(base))return'gatheringGear';
  if(/_MOUNT_/.test(base))return'mounts';
  if(/_TOOL_/.test(base))return'tools';
  if(/^T[2-8]_(?:RUNE|SOUL|RELIC|ESSENCE)(?:_|$)/.test(base))return'bulkMaterials';
...
function isT3ResourceId(id){const te=itemTierEnchant(id),cat=categoryForItem(id);return te.tier===3&&(cat==='rawResources'||cat==='refinedResources');}
function resourceTypeLabel(id){return categoryForItem(id)==='rawResources'?tr('podstawowy','raw'):tr('przetworzony','refined');}
function bestPriceRow(rows,field,direction){return (rows||[]).filter(r=>(+r[field]||0)>0).sort((a,b)=>direction*((+a[field]||0)-(+b[field]||0))||(parseAodpTime(b[field+'_date'])||0)-(parseAodpTime(a[field+'_date'])||0))[0]||null;}
function buildT3ResourceCoverage(items,rows,failedIds=new Set(),opportunities=[],scanned=true){
  const byItem=new Map();for(const r of rows||[]){if(!byItem.has(r.item_id))byItem.set(r.item_id,[]);byItem.get(r.item_id).push(r);}
  const opsByItem=new Map();for(const o of opportunities||[]){if(!opsByItem.has(o.itemId)||(+o.profit||0)>(+opsByItem.get(o.itemId).profit||0))opsByItem.set(o.itemId,o);}
  return (items||[]).filter(x=>isT3ResourceId(x.id)).map(item=>{
    const itemRows=byItem.get(item.id)||[],buy=bestPriceRow(itemRows,'sell_price_min',1),instantSell=bestPriceRow(itemRows,'buy_price_max',-1),opportunity=opsByItem.get(item.id)||null;
    const timestamps=[buy?.sell_price_min_date,instantSell?.buy_price_max_date].map(parseAodpTime).filter(Number.isFinite),newestTs=timestamps.length?Math.max(...timestamps):0;
    let status='pending';if(scanned){if(failedIds.has(item.id)&&(buy||instantSell))status='cache-api-error';else if(failedIds.has(item.id))status='api-error';else if(!buy&&!instantSell)status='no-prices';else if(opportunity)status='profitable';else if(!buy)status='no-purchase-price';else status='no-profit';}
    return{itemId:item.id,item,buy,instantSell,opportunity,status,newestTs,quoteRows:itemRows.length};
  }).sort((a,b)=>b.newestTs-a.newestTs||a.itemId.localeCompare(b.itemId));
}
...
function openLocalDB(){
  return new Promise((resolve,reject)=>{
    if(!globalThis.indexedDB)return reject(new Error('IndexedDB niedostępna'));
    let finished=false;
    const finish=(fn,value)=>{if(finished)return;finished=true;clearTimeout(timer);fn(value);};
    const timer=setTimeout(()=>finish(reject,new Error('Przekroczono czas otwierania IndexedDB')),8000);
    let req;try{req=indexedDB.open(DB_NAME,DB_VERSION);}catch(error){finish(reject,error);return;}
    req.onupgradeneeded=()=>{
      const d=req.result;
      ...
      if(d.objectStoreNames.contains('market_stats'))d.deleteObjectStore('market_stats');
      if(d.objectStoreNames.contains('portfolios'))d.deleteObjectStore('portfolios');
    };
    req.onsuccess=()=>{if(finished){req.result.close();return;}db=req.result;db.onversionchange=()=>{db?.close();db=null;};finish(resolve,db);};
    req.onerror=()=>finish(reject,req.error||new Error('Nie udało się otworzyć IndexedDB'));
    req.onblocked=()=>finish(reject,new Error('IndexedDB jest zablokowana przez inną kartę'));
  });
}
...
function memoryKey(store,value){if(store==='items')return value.id;if(store==='watchlist')return value.itemId;if(store==='scan_runs')return value.id??++memoryAutoId;return value.key;}
async function dbPut(store,value){if(!db){memoryStores.get(store)?.set(memoryKey(store,value),value);return;}const tx=db.transaction(store,'readwrite');tx.objectStore(store).put(value);await txDone(tx);}
async function dbPutMany(store,values){if(!values?.length)return;if(!db){for(const value of values)memoryStores.get(store)?.set(memoryKey(store,value),value);return;}const tx=db.transaction(store,'readwrite'),s=tx.objectStore(store);for(const v of values)s.put(v);await txDone(tx);}
async function dbGet(store,key){if(!db)return memoryStores.get(store)?.get(key);return new Promise((resolve,reject)=>{const r=db.transaction(store).objectStore(store).get(key);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error);});}
async function dbGetAll(store){if(!db)return[...(memoryStores.get(store)?.values()||[])];return new Promise((resolve,reject)=>{const r=db.transaction(store).objectStore(store).getAll();r.onsuccess=()=>resolve(r.result||[]);r.onerror=()=>reject(r.error);});}
async function dbDeleteMany(store,keys){if(!keys?.length)return;if(!db){const s=memoryStores.get(store);for(const key of keys)s?.delete(key);return;}const tx=db.transaction(store,'readwrite'),s=tx.objectStore(store);for(const key of keys)s.delete(key);await txDone(tx);}
async function dbClear(store){if(!db){memoryStores.get(store)?.clear();return;}const tx=db.transaction(store,'readwrite');tx.objectStore(store).clear();await txDone(tx);}
...
async function updateDbInfo(){
  ...
  const label=db?'IndexedDB':(state.lang==='en'?'Temporary memory':'Pamięć tymczasowa');
  el('localDbInfo').textContent=state.lang==='en'?`${label}: ${fmt(prices.length)} prices • ${fmt(hist.length)} volume histories • ${fmt(ops.length)} routes${newest?' • cache '+new Date(newest).toLocaleString(locale()):''}.`:`${label}: ${fmt(prices.length)} cen • ${fmt(hist.length)} historii wolumenu • ${fmt(ops.length)} tras${newest?' • cache '+new Date(newest).toLocaleString(locale()):''}.`;
}
...
function currentSettings(){
  ...
  v.markets=selectedMarkets();v.categories=selectedCategories();v.sortDirection=state.sortDirection;v._uiSchema=13;return v;
}
async function saveSettings(){await dbPut('settings',{key:'ui',value:currentSettings(),savedAt:now()});scheduleAuto();}
async function loadSettings(){
  ...
  // v5.5.4: price columns sort by their observation time, newest first.
  if((+v._uiSchema||1)<13){if(v.sortBy==='buy')v.sortBy='buyTime';if(v.sortBy==='sell')v.sortBy='sellTime';if(['buyTime','sellTime'].includes(v.sortBy))v.sortDirection='desc';v._uiSchema=13;}
...
function isLikelyTradeable(id){if(isLuxuryGoodId(id))return true;if(!/^T[2-8]_/.test(id))return false;const bad=['QUESTITEM','UNIQUE','TOKEN','SKILLBOOK','JOURNAL_EMPTY','FURNITURE_','MOBDROP_','TREASURE_','FACTION_','CRYSTAL_','DEBUG','NONTRADABLE'];return !bad.some(k=>id.includes(k));}
...
async function loadItems(){
  ...
  if(cached.length&&cached.some(x=>x.nameEn)&&meta?.catalogSchema===ITEM_CATALOG_SCHEMA&&meta?.fetchedAt&&now()-meta.fetchedAt<7*864e5){const items=withLuxuryFallbacks(cached);setItems(items);el('dbStatus').textContent=fmt(items.length);return;}
  for(const url of ITEMS_URLS){try{const j=await fetchJson(url,25000),arr=Array.isArray(j)?j:(j.items||Object.values(j));let items=arr.map(normalizeItem).filter(Boolean).filter(x=>isLikelyTradeable(x.id));items=withLuxuryFallbacks([...new Map(items.map(x=>[x.id,x])).values()]);setItems(items);await dbClear('items');await dbPutMany('items',items);await dbPut('settings',{key:'items_meta',fetchedAt:now(),source:url,catalogSchema:ITEM_CATALOG_SCHEMA});el('dbStatus').textContent=fmt(items.length);return;}catch(e){lastErr=e;}}
  if(cached.length){const items=withLuxuryFallbacks(cached);setItems(items);el('dbStatus').textContent=fmt(items.length);return;}throw lastErr||new Error('Nie udało się pobrać bazy przedmiotów');
}
...
function liquidItemPriority(id){
  ...
  let cat=0;if(/_(RUNE|SOUL|RELIC|ESSENCE)(?:_|$)/.test(id))cat=10;else if(isLuxuryGoodId(id))cat=8;else if(/_(ORE|WOOD|HIDE|FIBER|ROCK|METALBAR|PLANKS|LEATHER|CLOTH|STONEBLOCK)/.test(id))cat=7;else if(/_(MEAL|FOOD|POTION)/.test(id))cat=6;else if(/_(BAG|CAPE)/.test(id))cat=5;else if(/_(MAIN_|2H_|ARMOR_|HEAD_|SHOES_|OFF_)/.test(id))cat=4;else if(/MOUNT/.test(id))cat=3;
...
function buildItemSelection(){
  ...
  let a=state.items.filter(it=>{const id=it.id,{tier:t,enchant:e}=itemTierEnchant(id),cat=categoryForItem(id),untiered=cat==='luxury';if(!untiered&&(t<2||t>8))return false;if(tier!=='all'&&(untiered||String(t)!==tier))return false;if(ench!=='all'&&String(e)!==ench)return false;if(!cats.has(cat))return false;const names=[it.name,it.namePl,it.nameEn,id].filter(Boolean).map(x=>String(x).toLowerCase());if(q&&!names.some(x=>x.includes(q)))return false;return true;});
...
function hasAnyQuote(r){return(+r.sell_price_min||0)>0||(+r.buy_price_max||0)>0||(+r.sell_price_max||0)>0||(+r.buy_price_min||0)>0;}
async function persistPriceRows(rows){const normalized=rows.map(r=>normalizePriceRow(r,'api')),useful=normalized.filter(hasAnyQuote),emptyKeys=normalized.filter(r=>!hasAnyQuote(r)).map(r=>r.key);await dbDeleteMany('prices',emptyKeys);await dbPutMany('prices',useful);return useful;}
  ...
  for(const [itemId,rows] of rowsByItem){
    ...
    for(const qRows of byQ.values()){
      ...
      const destinations=candidateDestinations.filter(r=>{
        const bad=r.city!=='Black Market'&&guard.isOutlier(r._destRawPrice);
      ...
      for(const b of buys){for(const d of destinations){
        ...
        const buy=Math.round(+b.sell_price_min),gross=mode==='relist'?Math.max(1,Math.floor(+d._destPrice)):Math.round(+d._destPrice);
        if(!(buy>0&&gross>0))continue;
        const tax=Math.ceil(gross*cfg.tax/100),setup=mode==='relist'?Math.ceil(gross*cfg.setup/100):0,fees=tax+setup;
        ...
        result.push({schemaVersion:17,key:`${itemId}|${b.quality}|${b.city}|${d.city}|${mode}`,itemId,itemName:itemDisplayName(item),itemNamePl:item.namePl||item.name,itemNameEn:item.nameEn||item.name,quality:+b.quality,source:b.city,dest:d.city,buy,sell:gross,rawDestinationPrice:d._destRawPrice,profit,fees,taxFee:tax,setupFee:setup,transportCost:cfg.transport,taxRatePct:cfg.tax,setupRatePct:mode==='relist'?cfg.setup:0,sourceDate:b.sell_price_min_date,destDate:d._destDate,mode,calculatedAt:now(),priceReference:{peerMedian:guard.median,peerCap:guard.cap,peerCount:guard.count,sourcePeerMedian:sourceGuard.median,sourcePeerFloor:sourceGuard.floor,sourcePeerCount:sourceGuard.count,sourcePeerLowOutlier:!!b._sourcePeerLow,blackMarketSeparated:d.city==='Black Market'},sourceRow:b,destRow:d});
        md.acceptedPairs++;
      }}
    }
  }
  result.sort((a,b)=>(b.profit||0)-(a.profit||0));md.signals=result.length;return result;
}
function calcLuxuryOpportunities(rowsByItem,itemMap,cfg,diag){
  const result=[],md=diag.modes.luxury={items:0,qualities:0,pairs:0,rejectSame:0,rejectNonPositive:0,rejectAnomaly:0,flagLowAnomaly:0,acceptedPairs:0,signals:0},startCity=cfg.startCity||'all';
  for(const [itemId,rows] of rowsByItem){
    const rule=luxuryFixedRule(itemId);if(!rule)continue;md.items++;
    const byQ=new Map();for(const r of rows){const q=+r.quality||1;if(!byQ.has(q))byQ.set(q,[]);byQ.get(q).push(r);}
    for(const qRows of byQ.values()){
      md.qualities++;
      const candidates=qRows.filter(r=>r.city!=='Black Market'&&+r.sell_price_min>0&&(startCity==='all'||r.city===startCity));
      md.pairs+=candidates.length;if(!candidates.length)continue;
      const guard=marketPriceGuard(candidates,'sell_price_min'),ranked=candidates.map(r=>({...r,_sourcePeerLow:guard.isLowOutlier(+r.sell_price_min)})).sort((a,b)=>(+a.sell_price_min)-(+b.sell_price_min)||parseAodpTime(b.sell_price_min_date)-parseAodpTime(a.sell_price_min_date));
      const b=ranked[0];if(b._sourcePeerLow)md.flagLowAnomaly++;
      const buy=Math.round(+b.sell_price_min),gross=rule.payout,tax=Math.ceil(gross*cfg.tax/100),profit=gross-tax-buy-cfg.transport;
      if(!(profit>0)){md.rejectNonPositive++;continue;}
      const item=itemMap.get(itemId)||{id:itemId,name:itemId,namePl:itemId,nameEn:itemId};
      result.push({schemaVersion:18,key:`${itemId}|${b.quality}|${b.city}|${rule.destination}|luxury`,itemId,itemName:itemDisplayName(item),itemNamePl:item.namePl||item.name,itemNameEn:item.nameEn||item.name,quality:+b.quality,source:b.city,dest:rule.destination,buy,sell:gross,rawDestinationPrice:gross,profit,fees:tax,taxFee:tax,setupFee:0,transportCost:cfg.transport,taxRatePct:cfg.tax,setupRatePct:0,sourceDate:b.sell_price_min_date,destDate:null,mode:'luxury',fixedPayout:true,anyRoyalDestination:!!rule.anyRoyal,calculatedAt:now(),priceReference:{sourcePeerMedian:guard.median,sourcePeerFloor:guard.floor,sourcePeerCount:guard.count,sourcePeerLowOutlier:!!b._sourcePeerLow},sourceRow:b,destRow:null});
      md.acceptedPairs++;
    }
  }
  result.sort((a,b)=>(b.profit||0)-(a.profit||0));md.signals=result.length;return result;
...
function calcOpportunities(rowsByItem,itemMap,mode,cfg){
  ...
  const luxuryRows=new Map([...rowsByItem].filter(([id])=>isLuxuryGoodId(id))),normalRows=new Map([...rowsByItem].filter(([id])=>!isLuxuryGoodId(id)));
  let out;if(mode==='smart')out=[...calcMode(normalRows,itemMap,'instant',cfg,diag),...calcMode(normalRows,itemMap,'relist',cfg,diag)];else out=calcMode(normalRows,itemMap,mode,cfg,diag);
  out.push(...calcLuxuryOpportunities(luxuryRows,itemMap,cfg,diag));
...
function historyMetrics(data,windowDays=30){
  const raw=(data||[]).map((x,i)=>({price:+x.avg_price||0,volume:Math.max(0,+x.item_count||0),time:parseHistoryTime(x.timestamp),i})).filter(x=>x.price>0||x.volume>0);if(!raw.length)return null;
  const validTimes=raw.filter(x=>Number.isFinite(x.time)),todayKey=utcDayKey(now()),dailyMap=new Map();
  if(validTimes.length){for(const x of validTimes){const k=utcDayKey(x.time),r=dailyMap.get(k)||{volume:0,priceVol:0,volForPrice:0,prices:[]};r.volume+=x.volume;if(x.price>0){r.prices.push(x.price);if(x.volume>0){r.priceVol+=x.price*x.volume;r.volForPrice+=x.volume;}}dailyMap.set(k,r);}}
  const complete=[];if(validTimes.length){for(let n=1;n<=windowDays;n++){const d=new Date(now()-n*864e5),k=utcDayKey(d.getTime()),r=dailyMap.get(k);complete.push({day:k,volume:r?.volume||0,price:r?(r.volForPrice>0?r.priceVol/r.volForPrice:median(r.prices.filter(Boolean))):NaN});}}else{const ordered=[...raw].slice(-windowDays).reverse();for(let n=0;n<windowDays;n++){const x=ordered[n];complete.push({day:`fallback-${n}`,volume:x?.volume||0,price:x?.price||NaN});}}
  const today=dailyMap.get(todayKey)?.volume||0,win=n=>{const a=complete.slice(0,n),vols=a.map(x=>x.volume),total=vols.reduce((p,c)=>p+c,0),active=vols.filter(v=>v>0).length,avg=total/Math.max(1,n),med=median(vols),activeRatio=active/Math.max(1,n);return{days:n,total,avg,medianDaily:Number.isFinite(med)?med:0,active,activeRatio};};
  const priceWin=n=>{const a=complete.slice(0,n).filter(x=>Number.isFinite(x.price)&&x.price>0),prices=a.map(x=>x.price),weighted=a.filter(x=>x.volume>0),weight=weighted.reduce((s,x)=>s+x.volume,0),vwap=weight>0?weighted.reduce((s,x)=>s+x.price*x.volume,0)/weight:mean(prices),med=prices.length?median(prices):NaN,mad=prices.length?median(prices.map(v=>Math.abs(v-med))):NaN,scale=Number.isFinite(mad)?1.4826*mad:NaN,lo=Number.isFinite(scale)&&scale>0?Math.max(1,med-3*scale):med,hi=Number.isFinite(scale)&&scale>0?med+3*scale:med,robustWeighted=a.filter(x=>x.volume>0).map(x=>({...x,wp:clamp(x.price,lo,hi)})),robustWeight=robustWeighted.reduce((s,x)=>s+x.volume,0),robustVwap=prices.length?(robustWeight>0?robustWeighted.reduce((s,x)=>s+x.wp*x.volume,0)/robustWeight:median(prices)):NaN;return{days:n,points:a.length,median:Number.isFinite(med)?med:null,avg:prices.length?mean(prices):null,vwap:Number.isFinite(vwap)?vwap:null,robustVwap:Number.isFinite(robustVwap)?robustVwap:null,mad:Number.isFinite(mad)?mad:null};};
  const w1=win(1),w3=win(3),w7=win(7),w14=win(14),w30=win(30),p3=priceWin(3),p7=priceWin(7),p14=priceWin(14),p30=priceWin(30),prices=raw.filter(x=>x.price>0).map(x=>x.price),volumes=raw.map(x=>x.volume),med=median(prices),avg=mean(prices),mad=median(prices.map(x=>Math.abs(x-med))),robustVol=med>0?(1.4826*mad/med):1,totalVol=w30.total,dailyVol=w30.avg,activeDailyVol=mean(volumes)||0,sumVol=raw.reduce((a,x)=>a+x.volume,0),vwap=sumVol>0?raw.reduce((a,x)=>a+x.price*x.volume,0)/sumVol:avg;
  const regularity=clamp((.65*w30.activeRatio+.35*w7.activeRatio)*100),trendPct=w14.avg>0?(w3.avg/w14.avg-1)*100:0,nz=complete.slice(0,30).map(x=>x.volume),volMean=mean(nz)||0,volSd=Math.sqrt(mean(nz.map(v=>(v-volMean)**2))||0),volumeCv=volMean>0?volSd/volMean:9,stability=clamp(100-volumeCv*32);
  return{median:med,avg,vwap,mad,robustVol,dailyVol,activeDailyVol,totalVol,windowDays,points:raw.length,todayVol:today,volumes:{d1:w1,d3:w3,d7:w7,d14:w14,d30:w30},prices:{d3:p3,d7:p7,d14:p14,d30:p30},regularity,trendPct,volumeCv,volumeStability:stability};
}
function conservativeMarketDaily(m){
  if(!m)return null;
  const a=+m?.volumes?.d7?.avg,med=+m?.volumes?.d7?.medianDaily;
  if(!Number.isFinite(a))return null;
  if(a<=0)return 0;
  // Deliberately conservative: a route should not be sized from a single high-volume day.
  // If fewer than half of the last 7 days were active, medianDaily can be zero and the safe cap is zero.
  if(Number.isFinite(med))return Math.max(0,Math.min(a,med));
  return Math.max(0,a);
}
...
async function loadVolumeHistoriesForOpportunities(ops,qualities,force=false){
  if(!ops?.length)return {cache:new Map(),failedKeys:new Set(),jobs:0,errors:0};
  ...
  for(const o of ops){
    ...
    if(!o.fixedPayout&&o.dest!=='Black Market'){if(!wantedByCity.has(o.dest))wantedByCity.set(o.dest,new Set());wantedByCity.get(o.dest).add(o.itemId);}
  }
  ...
  const failedKeys=new Set();let next=0,completed=0,errors=0;
  const processJob=async(job,idx)=>{
    ...
    if(rows!==null){
      ...
      for(const h of rows){const itemId=String(h.item_id||''),city=String(h.location||job.city),q=+h.quality||1;if(!itemId)continue;const rec={key:volumeHistoryKey(itemId,city,q),itemId,city,quality:q,data:Array.isArray(h.data)?h.data:[],rangeDays:32,timeScale:24,fetchedAt:now(),confirmed:true};returned.set(rec.key,rec);cache.set(rec.key,rec);}
      const writes=[...returned.values()];
      // Only a successful API response may confirm a missing item/quality as zero volume.
      for(const id of job.batch){for(const q of qualities){const key=volumeHistoryKey(id,job.city,q);if(!returned.has(key)){const rec={key,itemId:id,city:job.city,quality:+q,data:[],rangeDays:32,timeScale:24,fetchedAt:now(),confirmed:true};writes.push(rec);cache.set(key,rec);}}}
      await dbPutMany('history',writes);
    }
...
function volumeRecordState(rec,failed){
  if(!rec)return failed?'error':'missing';
  const age=Number.isFinite(+rec.fetchedAt)?now()-+rec.fetchedAt:Infinity;
  if(failed)return age<4*36e5?'error-cache':'stale-cache';
  return age<4*36e5?'fresh':'stale-cache';
}
function attachSimpleVolume(o,cache,failedKeys=new Set()){
  const fixed=!!o.fixedPayout,srcKey=volumeHistoryKey(o.itemId,o.source,o.quality),dstKey=(fixed||o.dest==='Black Market')?null:volumeHistoryKey(o.itemId,o.dest,o.quality),srcRec=cache.get(srcKey),dstRec=dstKey?cache.get(dstKey):null,srcState=volumeRecordState(srcRec,failedKeys.has(srcKey)),dstState=fixed?'fixed':o.dest==='Black Market'?'unavailable':volumeRecordState(dstRec,failedKeys.has(dstKey)),sm=srcRec?historyMetrics(srcRec.data||[]):null,dm=dstRec?historyMetrics(dstRec.data||[]):null;
  const buyDay=sm?.volumes?.d7?.avg??(srcRec?.confirmed?0:null),sellDay=(fixed||o.dest==='Black Market')?null:(dm?.volumes?.d7?.avg??(dstRec?.confirmed?0:null));
  const sourceSafeDaily=srcRec?.confirmed?(conservativeMarketDaily(sm)??0):null,destinationSafeDaily=(fixed||o.dest==='Black Market')?null:(dstRec?.confirmed?(conservativeMarketDaily(dm)??0):null);
  const routeSafeDaily=fixed?sourceSafeDaily:(Number.isFinite(sourceSafeDaily)&&Number.isFinite(destinationSafeDaily)?Math.min(sourceSafeDaily,destinationSafeDaily):null);
  const src7=sm?.volumes?.d7?.total??(srcRec?.confirmed?0:null),dst7=(fixed||o.dest==='Black Market')?null:(dm?.volumes?.d7?.total??(dstRec?.confirmed?0:null)),src30=sm?.volumes?.d30?.total??(srcRec?.confirmed?0:null),dst30=(fixed||o.dest==='Black Market')?null:(dm?.volumes?.d30?.total??(dstRec?.confirmed?0:null)),trade7=fixed?src7:(Number.isFinite(src7)&&Number.isFinite(dst7)?Math.min(src7,dst7):null),trade30=fixed?src30:(Number.isFinite(src30)&&Number.isFinite(dst30)?Math.min(src30,dst30):null);
  // effectiveDaily now means conservative destination sell capacity. routeSafeDaily is the two-sided route scale.
  o.volume={source:sm?.volumes||null,destination:dm?.volumes||null,buyDaily:buyDay,sellDaily:sellDay,sourceSafeDaily,destinationSafeDaily,routeSafeDaily,effectiveDaily:fixed?sourceSafeDaily:destinationSafeDaily,trade7,trade30,profitVolumeDaily:0,sourceStatus:srcState,destinationStatus:dstState};
  const destinationHistoryOutlier=!fixed&&o.dest!=='Black Market'&&historyPriceOutlier(o.sell,dm,'high'),sourceHistoryLowWarning=historyPriceOutlier(o.buy,sm,'low'),sourcePeerLowWarning=!!o.priceReference?.sourcePeerLowOutlier,sourceWarning=sourcePeerLowWarning||sourceHistoryLowWarning;
  o.priceGuard={...(o.priceReference||{}),historyMedian:dm?.median??null,historyReference:historyReferencePrice(dm),sourceHistoryMedian:sm?.median??null,sourceHistoryReference:historyReferencePrice(sm),historyOutlier:destinationHistoryOutlier,destinationHistoryOutlier,sourceHistoryLowWarning,sourcePeerLowWarning,sourceWarning};
  // Only an implausibly HIGH destination is hard-rejected. A very low source is retained and flagged.
  o.invalidPrice=destinationHistoryOutlier;
  o.volume.profitVolumeDaily=destinationHistoryOutlier?0:(Number.isFinite(routeSafeDaily)?Math.max(0,o.profit)*routeSafeDaily:0);o.profitVolumeDaily=o.volume.profitVolumeDaily;o.history={source:sm,destination:dm,points:srcRec?.data||[],basis:'AODP sell history; median-first robust 7/14/30d references; not current order-book depth'};o.reality=marketRealityAssessment(o,sm,dm);return o;
}
async function enrichAllVolume(force=false){
  ...
  try{const result=await loadVolumeHistoriesForOpportunities(state.opportunities,qualities,force);state.volumeFailures=result.failedKeys;state.volumeStats={jobs:result.jobs,errors:result.errors};for(const o of state.opportunities)attachSimpleVolume(o,result.cache,result.failedKeys);const before=state.opportunities.length;state.opportunities=state.opportunities.filter(o=>!o.invalidPrice);markRouteRanks(state.opportunities);const removed=before-state.opportunities.length;if(state.scanDiagnostics)state.scanDiagnostics.historyPriceAnomalies=(state.scanDiagnostics.historyPriceAnomalies||0)+removed;const coverageItems=state.t3ResourceCoverage.map(x=>x.item);state.t3ResourceCoverage=buildT3ResourceCoverage(coverageItems,state.raw,state.t3FailedPriceIds,state.opportunities,true);await saveOpportunities(state.opportunities);state.page=1;render();await updateDbInfo();}finally{el('validateBtn').disabled=false;el('validateBtn').textContent=tr('Odśwież wolumen','Refresh volume');}
}

function diagnosticTotals(){
  const d=state.scanDiagnostics||{},m=Object.values(d.modes||{}),sum=k=>m.reduce((a,x)=>a+(x[k]||0),0);return{pairs:sum('pairs'),same:sum('rejectSame'),nonPositive:sum('rejectNonPositive'),anomaly:sum('rejectAnomaly'),lowWarnings:sum('flagLowAnomaly'),accepted:sum('acceptedPairs'),signals:sum('signals')};
}
function renderScanDiagnostics(){
  ...
  const volErr=state.volumeStats?.errors||0,volJobs=state.volumeStats?.jobs||0,histAnom=d.historyPriceAnomalies||0;const msg=state.lang==='en'?`Selected ${fmt(d.selectedItems||0)} / matching ${fmt(d.matchingItems||0)} • API rows ${fmt(d.apiRows||0)} • item IDs with data ${fmt(d.itemsWithRows||0)} • pairs ${fmt(t.pairs)} • high price anomalies rejected ${fmt((t.anomaly||0)+histAnom)} • low-price warnings ${fmt(t.lowWarnings||0)} • non-positive ${fmt(t.nonPositive)} • profitable routes ${fmt(state.opportunities.length)} • shown ${fmt(shown)} • volume batches ${fmt(volJobs)} • volume API errors ${fmt(volErr)}`:`Wybrano ${fmt(d.selectedItems||0)} z ${fmt(d.matchingItems||0)} pasujących • rekordy API ${fmt(d.apiRows||0)} • itemy z danymi ${fmt(d.itemsWithRows||0)} • pary ${fmt(t.pairs)} • odrzucone wysokie anomalie ${fmt((t.anomaly||0)+histAnom)} • ostrzeżenia niskiej ceny ${fmt(t.lowWarnings||0)} • niedodatnie ${fmt(t.nonPositive)} • zyskowne trasy ${fmt(state.opportunities.length)} • pokazane ${fmt(shown)} • partie wolumenu ${fmt(volJobs)} • błędy API wolumenu ${fmt(volErr)}`;
...
function countMatchingItems(){const q=el('itemQuery').value.trim().toLowerCase(),tier=el('tier').value,ench=el('enchant').value,cats=new Set(selectedCategories());return state.items.filter(it=>{const id=it.id,{tier:t,enchant:e}=itemTierEnchant(id),cat=categoryForItem(id),untiered=cat==='luxury';if(!untiered&&(t<2||t>8))return false;if(tier!=='all'&&(untiered||String(t)!==tier))return false;if(ench!=='all'&&String(e)!==ench)return false;if(!cats.has(cat))return false;if(q){const names=[it.name,it.namePl,it.nameEn,id].filter(Boolean).map(x=>String(x).toLowerCase());if(!names.some(x=>x.includes(q)))return false;}return true;}).length;}

async function scan(opts={}){
  if(opts.auto&&!state.manualScanUnlocked)return;
  if(el('scanBtn').disabled&&!opts.auto)return;ensureRouteMarketsSelected();const items=buildItemSelection();if(!items.length){if(!opts.auto)alert(tr('Brak przedmiotów pasujących do filtrów.','No items match the selected filters/categories.'));return;}
  const onlyLuxury=items.every(x=>isLuxuryGoodId(x.id)),markets=selectedMarkets(),minimumMarkets=onlyLuxury?1:2;if(markets.length<minimumMarkets){if(!opts.auto)alert(onlyLuxury?tr('Wybierz co najmniej jeden market zakupu.','Select at least one purchase market.'):tr('Wybierz co najmniej dwa markety.','Select at least two markets.'));return;}
  if(!onlyLuxury&&el('excludeSame').checked&&el('startCity').value!=='all'&&el('startCity').value===el('destinationCity').value){if(!opts.auto)alert(tr('Miasto startowe i docelowe muszą być różne.','Starting and destination cities must be different.'));return;}
  if(!opts.auto){state.manualScanUnlocked=true;scheduleAuto();}
  state.scanDiagnostics={selectedItems:items.length,matchingItems:countMatchingItems(),apiRows:0,itemsWithRows:0,modes:{}};state.volumeStats={jobs:0,errors:0};state.volumeFailures.clear();state.page=1;state.t3ResourceCoverage=buildT3ResourceCoverage(items,[],new Set(),[],false);renderT3ResourceCoverage();
  ...
  const normalizedFresh=[];const failedPriceItemIds=new Set();state.t3FailedPriceIds=failedPriceItemIds;let errors=0,processedItems=0,rawApiRows=0,usefulApiRows=0;
  ...
  state.scanDiagnostics.apiRows=usefulApiRows;state.scanDiagnostics.rawApiRows=rawApiRows;
  // Cache is allowed ONLY for item IDs belonging to a price batch that actually failed.
  // A successful zero/no-order response therefore cannot resurrect an older positive quote from local.db.
  let cached=[];if(el('useCache').checked&&failedPriceItemIds.size){const failedItems=items.filter(x=>failedPriceItemIds.has(x.id));cached=await cachedRowsFor(failedItems,markets,qualities);}const rows=mergeRows(normalizedFresh,cached);state.raw=rows;
  if(!rows.length){state.t3ResourceCoverage=buildT3ResourceCoverage(items,[],failedPriceItemIds,[],true);el('scanBtn').disabled=false;el('stopBtn').disabled=true;setProgress(0,tr('Brak danych API i brak lokalnego cache.','No API data and no local cache.'));render();await updateDbInfo();return;}
  ...
  const cfg=cfgFromUI();state.opportunities=calcOpportunities(byItem,map,el('mode').value,cfg);markRouteRanks(state.opportunities);state.t3ResourceCoverage=buildT3ResourceCoverage(items,rows,failedPriceItemIds,state.opportunities,true);render();renderScanDiagnostics();
  ...
  await dbPut('scan_runs',{ts:now(),items:items.length,markets,qualities,apiRows:usefulApiRows,rawApiRows,cacheRows:cached.length,failedPriceItems:failedPriceItemIds.size,errors,opportunities:state.opportunities.length,auto:!!opts.auto});
...
function orderTotalsForQty(o,qty){
  qty=Math.max(0,Math.floor(qty||0));
  const buyCost=qty*Math.max(0,+o.buy||0),revenue=qty*Math.max(0,+o.sell||0),transportTotal=qty*Math.max(0,+o.transportCost||0),taxTotal=qty>0?Math.ceil(revenue*Math.max(0,+o.taxRatePct||0)/100):0,setupTotal=o.mode==='relist'&&qty>0?Math.ceil(revenue*Math.max(0,+o.setupRatePct||0)/100):0;
  const capital=buyCost+transportTotal+setupTotal,totalProfit=revenue-taxTotal-setupTotal-buyCost-transportTotal;
  return{qty,buyCost,revenue,transportTotal,taxTotal,setupTotal,capital,totalProfit,effectiveProfitUnit:qty>0?totalProfit/qty:0};
}
function budgetQtyCapFor(o,budget,maxQty){
  maxQty=Math.max(0,Math.floor(maxQty||0));if(!(budget>0)||!maxQty)return 0;
  let lo=0,hi=maxQty;while(lo<hi){const mid=Math.ceil((lo+hi)/2),need=orderTotalsForQty(o,mid).capital;if(need<=budget)lo=mid;else hi=mid-1;}return lo;
}
function bulkPlanFor(o,cfg){
  const sourceDaily=Number.isFinite(o.volume?.sourceSafeDaily)?Math.max(0,o.volume.sourceSafeDaily):Number.isFinite(o.volume?.buyDaily)?Math.max(0,o.volume.buyDaily):Number.isFinite(o.volume?.effectiveDaily)?Math.max(0,o.volume.effectiveDaily):0;
  const destinationDaily=Number.isFinite(o.volume?.destinationSafeDaily)?Math.max(0,o.volume.destinationSafeDaily):Number.isFinite(o.volume?.sellDaily)?Math.max(0,o.volume.sellDaily):Number.isFinite(o.volume?.effectiveDaily)?Math.max(0,o.volume.effectiveDaily):0;
  const share=clamp(cfg.bulkVolumeShare,1,100)/100;
  // One trip is capped by roughly one conservative day of source-market activity: history is not live order-book depth.
  const sourceCap=sourceDaily>0?Math.max(0,Math.floor(sourceDaily)):0;
  // Destination determines how much we intentionally try to sell during the chosen horizon.
  // City buyback for luxury goods is fixed; route size is limited by the source only.
  const destinationCap=o.fixedPayout?sourceCap:(destinationDaily>0?Math.max(0,Math.floor(destinationDaily*cfg.bulkDays*share)):0);
  const volumeCap=Math.min(sourceCap,destinationCap);
  const maxQtyCap=Math.max(0,Math.floor(cfg.bulkMaxQty||0));
  const routeCap=Math.max(0,Math.min(volumeCap,maxQtyCap));
  const budgetCap=budgetQtyCapFor(o,cfg.bulkBudget,routeCap),qty=Math.max(0,Math.min(routeCap,budgetCap));
  const totals=orderTotalsForQty(o,qty),approxUnitCapital=Math.max(1,(+o.buy||0)+(+o.transportCost||0)+(o.mode==='relist'?(+o.sell||0)*(+o.setupRatePct||0)/100:0));
  const sellRate=o.fixedPayout?sourceDaily:destinationDaily*share;
  return{...totals,unitCapital:qty>0?totals.capital/qty:approxUnitCapital,daysAtVolume:sellRate>0?qty/sellRate:null,daysAtFullVolume:destinationDaily>0?qty/destinationDaily:null,sourceCap,destinationCap,volumeCap,budgetCap,maxQtyCap,volumeShare:share,sourceDaily,destinationDaily};
}
...
function applyT3ResourcesPreset(){
  setSelectedCategories(['rawResources','refinedResources']);
  el('itemQuery').value='';el('tier').value='3';el('enchant').value='all';el('quality').value='1';el('mode').value='smart';el('scanLimit').value='0';el('startCity').value='all';el('destinationCity').value='all';el('excludeSame').checked=true;
  document.querySelectorAll('.marketCheck').forEach(x=>{const market=MARKETS.find(m=>m.api===x.value);x.checked=!!market?.royal;});
  el('maxBuyPrice').value='0';el('minProfit').value='0';el('minVolumeDay').value='0';el('sortBy').value='freshness';state.sortDirection='desc';state.page=1;
  state.t3ResourceCoverage=buildT3ResourceCoverage(buildItemSelection(),[],new Set(),[],false);queueSaveSettings();render();
  setProgress(0,tr(`Preset T3 gotowy: ${fmt(state.t3ResourceCoverage.length)} surowców. Kliknij „Skanuj” ręcznie.`,`T3 preset ready: ${fmt(state.t3ResourceCoverage.length)} resources. Click Scan manually.`));
}

function t3CoverageStatus(row){
  const labels={pending:[tr('oczekuje na ręczny skan','waiting for manual scan'),'info'],profitable:[tr('zyskowna trasa','profitable route'),'good'],'no-profit':[tr('cena jest, brak dodatniego zysku','priced, no positive profit'),'warn'],'no-prices':[tr('brak ceny w AODP','no AODP price'),'bad'],'no-purchase-price':[tr('brak ceny zakupu','no purchase price'),'bad'],'cache-api-error':[tr('cena z cache — błąd API','cached price — API error'),'warn'],'api-error':[tr('błąd API dla produktu','item API error'),'bad']};
  return labels[row.status]||[row.status,'info'];
}
function renderT3ResourceCoverage(){
  const section=el('t3CoverageSection');if(!section)return;const rows=state.t3ResourceCoverage||[];section.hidden=!rows.length;if(!rows.length)return;
  const counts={};for(const r of rows)counts[r.status]=(counts[r.status]||0)+1;
  el('t3CoverageSummary').innerHTML=`<span class="tag info">${tr('Razem','Total')}: ${fmt(rows.length)}</span><span class="tag good">${tr('Zyskowne','Profitable')}: ${fmt(counts.profitable||0)}</span><span class="tag warn">${tr('Bez zysku','No profit')}: ${fmt(counts['no-profit']||0)}</span><span class="tag bad">${tr('Bez ceny / błąd','No price / error')}: ${fmt((counts['no-prices']||0)+(counts['no-purchase-price']||0)+(counts['api-error']||0)+(counts['cache-api-error']||0))}</span>`;
  el('t3CoverageBody').innerHTML=rows.map(r=>{const te=itemTierEnchant(r.itemId),status=t3CoverageStatus(r),op=r.opportunity;return `<tr><td><div class="itemcell"><img loading="lazy" src="${itemIcon(r.itemId,1)}" alt=""><div><div class="itemname">${esc(itemDisplayName(r.item))}</div><div class="itemid">${esc(r.itemId)}</div></div></div></td><td>${esc(resourceTypeLabel(r.itemId))}</td><td>${te.enchant?'.'+te.enchant:'.0'}</td><td>${r.buy?`<span class="coverage-price">${fmt(r.buy.sell_price_min)}</span><div class="itemid">${esc(marketLabel(r.buy.city))}</div>`:'—'}</td><td>${r.buy?esc(quoteTimestampText(r.buy.sell_price_min_date)):'—'}</td><td>${r.instantSell?`<span class="coverage-price">${fmt(r.instantSell.buy_price_max)}</span><div class="itemid">${esc(marketLabel(r.instantSell.city))}</div>`:'—'}</td><td>${r.instantSell?esc(quoteTimestampText(r.instantSell.buy_price_max_date)):'—'}</td><td>${op?`<b>${esc(marketLabel(op.source))} → ${esc(marketLabel(op.dest))}</b><div class="profit">+${fmt(op.profit)} / szt.</div>`:'—'}</td><td><span class="tag ${status[1]}">${esc(status[0])}</span></td></tr>`;}).join('');
  el('t3CoverageEmpty').hidden=!!rows.length;
}

function filteredResults(){
  let a=[...state.opportunities],q=el('resultSearch').value.trim().toLowerCase(),cfg=cfgFromUI();
  if(q)a=a.filter(o=>[displayOpportunityName(o),o.itemNamePl,o.itemNameEn,o.itemId,o.source,o.dest].some(x=>String(x).toLowerCase().includes(q)));
  if(state.watchOnly)a=a.filter(o=>state.watched.has(o.itemId));
  if(cfg.maxBuyPrice>0)a=a.filter(o=>(o.buy||Infinity)<=cfg.maxBuyPrice);
...
function volumeFmt(v,status='fresh'){
  ...
  const n=v.toLocaleString(locale(),{maximumFractionDigits:1});
  return (status==='error-cache'||status==='stale-cache')?`${n} ${tr('(cache)','(cache)')}`:n;
}
function simpleModeLabel(o){if(o.fixedPayout)return tr('Dobro — stała cena','Luxury — fixed payout');if(o.dest==='Black Market')return 'Black Market';return o.mode==='instant'?tr('Do buy orderu','To buy order'):tr('Wystaw sell order','List sell order');}
function render(){
  renderT3ResourceCoverage();
  const all=filteredResults(),pageSize=Math.max(25,+el('pageSize')?.value||state.pageSize||100);state.pageSize=pageSize;
  const pages=Math.max(1,Math.ceil(all.length/pageSize));state.page=Math.max(1,Math.min(state.page,pages));const start=(state.page-1)*pageSize,pageRows=all.slice(start,start+pageSize);
  if(el('resultCountMeta'))el('resultCountMeta').textContent=state.lang==='en'?`${fmt(all.length)} shown / ${fmt(state.opportunities.length)} profitable routes • page ${state.page}/${pages}`:`${fmt(all.length)} pokazanych / ${fmt(state.opportunities.length)} zyskownych tras • strona ${state.page}/${pages}`;
  el('emptyState').style.display=all.length?'none':'block';
  el('resultsBody').innerHTML=pageRows.map((o,i)=>{const v=o.volume||{},bulk=o.bulk||bulkPlanFor(o,cfgFromUI()),r=o.reality||{},gc=r.grade==='A'?'good':r.grade==='B'?'info':r.grade==='C'?'warn':'bad';return `<tr><td><button class="star ${state.watched.has(o.itemId)?'on':''}" data-watch="${esc(o.itemId)}">★</button></td><td><div class="itemcell"><img loading="lazy" src="${itemIcon(o.itemId,o.quality)}" alt=""><div><div class="itemname">${esc(displayOpportunityName(o))}</div><div class="itemid">${esc(o.itemId)} • ${qualityName[o.quality]||o.quality}</div></div></div></td><td><span class="tag info">${esc(simpleModeLabel(o))}</span></td><td class="route"><b>${esc(marketLabel(o.source))}</b>${o.bestSource?` <span class="tag good">${tr('najtańszy','cheapest')}</span>`:''}${o.priceGuard?.sourceWarning?` <span class="tag warn">${tr('sprawdź cenę','verify price')}</span>`:''}</td><td>${fmt(o.buy)}<div class="itemid quote-time">${esc(quoteTimestampText(o.sourceDate))}</div></td><td class="route"><b>${esc(marketLabel(o.dest))}</b>${o.bestDestination?` <span class="tag good">${tr('#1 sprzedaż','#1 sell')}</span>`:''}</td><td>${fmt(o.sell)}<div class="itemid quote-time">${esc(saleTimestampText(o))}</div></td><td class="profit">+${fmt(o.profit)}</td><td><span class="tag ${gc}">${r.grade||'—'} ${esc(r.label||'')}</span><div class="itemid">${Number.isFinite(r.score)?r.score+'/100':'—'} • ${tr('dane','data')} ${Number.isFinite(r.dataCompleteness)?r.dataCompleteness+'%':'—'} • ${ageText(r.worstAge)}</div></td><td>${Number.isFinite(r.sellRef)?fmt(r.sellRef):'—'}</td><td><b>${volumeFmt(v.sourceSafeDaily,v.sourceStatus)}</b><div class="itemid">${Number.isFinite(v.buyDaily)?tr('śr. ','avg. ')+volumeFmt(v.buyDaily,v.sourceStatus):''}</div></td><td><b>${destinationVolumeText(o)}</b><div class="itemid">${Number.isFinite(v.sellDaily)?tr('śr. ','avg. ')+volumeFmt(v.sellDaily,v.destinationStatus):''}</div></td><td><b>${fmt(bulk.qty)}</b><div class="itemid">${tr('cel','target')} ${Math.round((bulk.volumeShare||0)*100)}%</div></td><td>${fmt(bulk.capital)}</td><td class="profit">+${fmt(bulk.totalProfit)}</td><td class="profit">${fmt(o.profitVolumeDaily||0)}</td><td><button class="btn" data-detail="${i}">${tr('Szczegóły','Details')}</button></td></tr>`;}).join('');
...
function openDetail(o){
  ...
  const bulk=o.bulk||bulkPlanFor(o,cfgFromUI()),r=o.reality||{},gc=r.grade==='A'?'good':r.grade==='B'?'info':r.grade==='C'?'warn':'bad';el('detailCards').innerHTML=`<div class="detail-card"><small>${tr('Cena kupna','Buy price')}</small><b>${fmt(o.buy)}</b></div><div class="detail-card"><small>${tr('Cena sprzedaży','Sell price')}</small><b>${fmt(o.sell)}</b></div><div class="detail-card"><small>${tr('Zysk top quote / szt.','Top-quote profit / unit')}</small><b class="profit">+${fmt(o.profit)}</b></div><div class="detail-card"><small>${tr('Ocena realności','Market reality')}</small><b><span class="tag ${gc}">${r.grade||'—'} ${esc(r.label||'')}</span> ${Number.isFinite(r.score)?r.score+'/100':'—'}</b></div><div class="detail-card"><small>${tr('Kompletność danych','Data completeness')}</small><b>${Number.isFinite(r.dataCompleteness)?r.dataCompleteness+'%':'—'}</b></div><div class="detail-card"><small>${tr('Ref. kupna 7d','7d buy ref.')}</small><b>${Number.isFinite(r.buyRef)?fmt(r.buyRef):'—'}</b></div><div class="detail-card"><small>${tr('Ref. sprzedaży 7d','7d sell ref.')}</small><b>${Number.isFinite(r.sellRef)?fmt(r.sellRef):'—'}</b></div><div class="detail-card"><small>${tr('Odczyt ceny kupna','Buy quote observed')}</small><b class="quote-time-detail">${esc(quoteTimestampText(o.sourceDate))}</b></div><div class="detail-card"><small>${tr('Odczyt ceny sprzedaży','Sell quote observed')}</small><b class="quote-time-detail">${esc(saleTimestampText(o))}</b></div><div class="detail-card"><small>${tr('Źródło bezpieczne / dzień','Safe source / day')}</small><b>${volumeFmt(v.sourceSafeDaily,v.sourceStatus)}</b></div><div class="detail-card"><small>${tr('Sprzedaż cel bezpieczna / dzień','Safe destination sales / day')}</small><b>${destinationVolumeText(o)}</b></div><div class="detail-card"><small>${tr('Skala trasy / dzień','Route scale / day')}</small><b>${volumeFmt(v.routeSafeDaily,(v.sourceStatus==='error'||v.destinationStatus==='error')?'error':(v.sourceStatus?.includes('cache')||v.destinationStatus?.includes('cache'))?'stale-cache':'fresh')}</b></div><div class="detail-card"><small>${tr('Plan szt.','Planned units')}</small><b>${fmt(bulk.qty)}</b></div><div class="detail-card"><small>${tr('Kapitał planu','Plan capital')}</small><b>${fmt(bulk.capital)}</b></div><div class="detail-card"><small>${tr('Szac. zysk transportu','Est. trip profit')}</small><b class="profit">+${fmt(bulk.totalProfit)}</b></div><div class="detail-card"><small>${tr('Efektywny zysk / szt. planu','Effective plan profit / unit')}</small><b class="profit">${bulk.qty>0?'+'+fmt(bulk.effectiveProfitUnit):'—'}</b></div><div class="detail-card"><small>${tr('Czas sprzedaży planu','Plan sell-through')}</small><b>${Number.isFinite(bulk.daysAtVolume)?bulk.daysAtVolume.toLocaleString(locale(),{maximumFractionDigits:2})+' d':'—'}</b></div><div class="detail-card"><small>${tr('Śr. źródło 7d / dzień','7d avg source / day')}</small><b>${volumeFmt(v.buyDaily,v.sourceStatus)}</b></div><div class="detail-card"><small>${tr('Śr. cel 7d / dzień','7d avg destination / day')}</small><b>${volumeFmt(v.sellDaily,v.destinationStatus)}</b></div><div class="detail-card"><small>${tr('Historyczny zysk / szt.','Historical profit / unit')}</small><b class="${Number.isFinite(r.repeatableProfit)&&r.repeatableProfit>0?'profit':''}">${Number.isFinite(r.repeatableProfit)?fmt(r.repeatableProfit):'—'}</b></div>`;
  const data=state.history.get(`${o.itemId}|${o.source}|${o.quality}`)||o.history?.points||[];drawChart(data,o.buy);const note=o.fixedPayout?tr('Stała cena sprzedaży wynika z reguły dobra luksusowego; AODP służy tylko do znalezienia najtańszej oferty zakupu. Skup miejski nie ogranicza partii.','The fixed sell payout comes from the luxury-good rule; AODP is used only to find the cheapest purchase offer. City buyback does not cap the batch.'):o.dest==='Black Market'?tr('Dla Black Market publiczna historia AODP nie daje porównywalnego wolumenu aktualnego buy orderu, dlatego wolumen sprzedaży celu i sugerowana partia są oznaczone jako brak.','For Black Market, public AODP history does not provide comparable current buy-order volume, so destination sell volume and suggested quantity are unavailable.'):tr('Wolumen źródła i celu jest liczony osobno z historii sell-side AODP. Bezpieczny wolumen to minimum średniej 7d i mediany dziennej 7d. Pole Zabierz ogranicza partia możliwa do kupna wg skali źródła, planowany udział sprzedaży w mieście docelowym, budżet i limit sztuk. To nadal nie jest bieżąca głębokość order booka.','Source and destination volume are calculated separately from AODP sell-side history. Safe daily volume is the lower of the 7-day average and 7-day daily median. Take quantity is capped by source-market scale, the selected share of destination sales, budget and max units. This is still not current order-book depth.'),quoteCaveat=o.fixedPayout?tr('Czas obok ceny zakupu pochodzi z AODP; cena sprzedaży jest stałą wartością dobra w grze.','The buy time comes from AODP; the sell price is the item’s fixed in-game payout.'):tr('Każda cena jest ostatnim odczytem AODP z czasu pokazanego obok, a nie gwarantowaną ceną bieżącą w grze.','Each price is the latest AODP observation from the displayed time, not a guaranteed current in-game price.');el('detailNote').innerHTML=`${tr('Kup','Buy')}: <b>${fmt(o.buy)}</b> — ${esc(marketLabel(o.source))}. ${tr('Odczyt','Observed')}: <b>${esc(quoteTimestampText(o.sourceDate))}</b>.<br>${tr('Sprzedaj','Sell')}: <b>${fmt(o.sell)}</b> — ${esc(marketLabel(o.dest))}. ${tr('Podstawa','Basis')}: <b>${esc(saleTimestampText(o))}</b>.<br>${tr('Zysk po opłatach','Profit after fees')}: <b class="profit">+${fmt(o.profit)}</b> / ${tr('szt.','unit')}.<br>${tr('Interpretacja v5.5.4','v5.5.4 interpretation')}: <b>${Number.isFinite(r.score)?r.score+'/100':'—'}</b> — ${esc(r.label||tr('brak oceny','not rated'))}. ${Number.isFinite(r.repeatableProfit)?`${tr('Historyczny spread 7d po opłatach','7d historical spread after fees')}: <b class="${r.repeatableProfit>0?'profit':''}">${fmt(r.repeatableProfit)}</b> / ${tr('szt.','unit')}.`:''}<br><span class="muted">${o.priceGuard?.sourceWarning?`<b>${tr('Uwaga: bardzo niska cena zakupu została zachowana, ale wymaga sprawdzenia w grze.','Warning: the extremely low buy price was kept, but should be verified in-game.')}</b> `:''}${esc(note)} ${esc(quoteCaveat)}</span>`;el('detailDialog').showModal();
}
...
async function loadCachedOpportunities(){
  ...
  // v5.4.4 accepts only the timestamp-aware opportunity schema so old cache rows cannot hide quote times.
  // Never resurrect v5.4.7 luxury routes whose destination used a changing AODP quote.
  ops=ops.filter(x=>x&&((x.schemaVersion===18&&x.fixedPayout)||(x.schemaVersion===17&&!isLuxuryGoodId(x.itemId))));
...
async function exportDB(){
  ...
  a.href=u;a.download=`albion-local-db-v5_5_6-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);
}
...
async function clearCache(){
  ...
  state.opportunities=[];state.t3ResourceCoverage=[];state.t3FailedPriceIds.clear();state.history.clear();state.volumeFailures.clear();state.volumeStats={jobs:0,errors:0};state.page=1;
...
function bind(){
  ...
  if(el('t3ResourcesCategories'))el('t3ResourcesCategories').onclick=applyT3ResourcesPreset;
  ...
  if(el('t3ResourcesPreset'))el('t3ResourcesPreset').onclick=applyT3ResourcesPreset;
  ...
  if(el('exportT3Btn'))el('exportT3Btn').onclick=exportT3ResourcesCsv;
...
async function init(){
  try{
    ...
    try{await openLocalDB();setDbStatus(tr('gotowa','ready'));}
    catch(dbError){db=null;console.warn('IndexedDB fallback',dbError);setDbStatus(tr('tryb pamięci','memory mode'),false);}
