...
function applyLanguage(lang,persist=true){
  ...
  document.title=state.lang==='en'?'Albion Europe Market Scanner v5.2.5 — Price & Volume':'Albion Europe Market Scanner v5.2.5 — Cena i Wolumen';
...
function currentSettings(){
  const ids=['languageSelect','itemQuery','tier','enchant','quality','scanLimit','mode','premium','setupFee','undercut','transport','minProfit','minVolumeDay','excludeSame','autoRefresh','autoStart','useCache','pageSize'];
  const v={};for(const id of ids){const x=el(id);if(x)v[id]=x.type==='checkbox'?x.checked:x.value;}
  v.markets=selectedMarkets();v.categories=selectedCategories();v._uiSchema=5;return v;
}
...
async function loadSettings(){
  ...
  const v={...rec.value};
  // v5.2.5: only price + volume are result criteria.
  if((+v._uiSchema||1)<5){Object.assign(v,{minProfit:'0',minVolumeDay:'0',quality:'all',pageSize:v.pageSize||'100',_uiSchema:5});}
...
function cfgFromUI(){return{minProfit:+el('minProfit')?.value||0,minVolumeDay:+el('minVolumeDay')?.value||0,excludeSame:!!el('excludeSame')?.checked,tax:el('premium').value==='yes'?4:8,setup:+el('setupFee').value||0,undercut:+el('undercut').value||0,transport:+el('transport').value||0};}
  ...
  for(const [itemId,rows] of rowsByItem){
    ...
    for(const qRows of byQ.values()){
      ...
      for(const b of buys){for(const d of destinations){
        ...
        result.push({schemaVersion:11,key:`${itemId}|${b.quality}|${b.city}|${d.city}|${mode}`,itemId,itemName:itemDisplayName(item),itemNamePl:item.namePl||item.name,itemNameEn:item.nameEn||item.name,quality:+b.quality,source:b.city,dest:d.city,buy,sell:gross,profit,fees,taxFee:tax,setupFee:setup,transportCost:cfg.transport,sourceDate:b.sell_price_min_date,destDate:d._destDate,mode,calculatedAt:now(),sourceRow:b,destRow:d});
        md.acceptedPairs++;
      }}
    }
  }
...
function render(){
  ...
  el('resultsBody').innerHTML=pageRows.map((o,i)=>{const v=o.volume||{},v7=Number.isFinite(v.trade7)?fmt(v.trade7):'—',v30=Number.isFinite(v.trade30)?fmt(v.trade30):'—';return `<tr><td><button class="star ${state.watched.has(o.itemId)?'on':''}" data-watch="${esc(o.itemId)}">★</button></td><td><div class="itemcell"><img loading="lazy" src="${itemIcon(o.itemId,o.quality)}" alt=""><div><div class="itemname">${esc(displayOpportunityName(o))}</div><div class="itemid">${esc(o.itemId)} • ${qualityName[o.quality]||o.quality}</div></div></div></td><td><span class="tag info">${esc(simpleModeLabel(o))}</span></td><td class="route"><b>${esc(marketLabel(o.source))}</b></td><td>${fmt(o.buy)}</td><td class="route"><b>${esc(marketLabel(o.dest))}</b></td><td>${fmt(o.sell)}</td><td class="profit">+${fmt(o.profit)}</td><td>${volumeFmt(v.buyDaily,v.sourceStatus)}</td><td>${volumeFmt(v.sellDaily,v.destinationStatus)}</td><td><b>${volumeFmt(v.effectiveDaily,(v.sourceStatus==='error'||v.destinationStatus==='error')?'error':(v.sourceStatus?.includes('cache')||v.destinationStatus?.includes('cache'))?'stale-cache':'fresh')}</b></td><td>${v7}</td><td>${v30}</td><td class="profit">${fmt(o.profitVolumeDaily||0)}</td><td><button class="btn" data-detail="${i}">${tr('Szczegóły','Details')}</button></td></tr>`;}).join('');
