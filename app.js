...
function applyLanguage(lang,persist=true){
  ...
  document.title=state.lang==='en'?'Albion Europe Market Scanner v5.4.8 — Luxury Fixed Value':'Albion Europe Market Scanner v5.4.8 — Dobra luksusowe';
...
async function loadSettings(){
  ...
  // v5.4.8: price columns sort by their observation time, newest first.
...
async function exportDB(){
  ...
  a.href=u;a.download=`albion-local-db-v5_4_8-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);
}
...
async function init(){
  ...
  if('serviceWorker' in navigator)navigator.serviceWorker.register('./service-worker.js?v=5.4.8').catch(()=>{});
...
function exportCsv(){
  ...
  const blob=new Blob(['\ufeff'+lines.join('\n')],{type:'text/csv;charset=utf-8'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='albion_europe_bulk_trade_v5_4_8.csv';a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);
}
init();
})();
