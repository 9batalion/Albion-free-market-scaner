...
const CATEGORY_GROUPS = [
  ...
  {id:'weaponsGroup',pl:'Broń',en:'Weapons',children:[['weapons','Broń główna i dwuręczna','Main & two-handed'],['offhands','Off-handy','Off-hands']]},
...
const EQUIPMENT_CATEGORIES = new Set(['head','chest','shoes','weapons','offhands','bags','capes','gatheringGear']);
...
async function enrichTop(limit,manual=false){
  ...
  for(let i=0;i<top.length;i++){if(state.stop)break;await enrichOne(top[i],cfg);setProgress(84+(i+1)/top.length*15,state.lang==='en'?`History models ${i+1}/${top.length} • liquidity • volatility • anomaly…`:`Modele historii ${i+1}/${top.length} • liquidity • volatility • anomaly…`);await sleep(170);}
...
async function scan(opts={}){
  ...
  if(!state.stop&&el('autoPortfolio')?.checked){setProgress(98,tr('Optymalizacja portfela według budżetu i ryzyka…','Optimizing portfolio by budget and risk…'));await buildPortfolio({skipEnrich:true,silent:true});}
...
function portfolioReason(o,cap){
  ...
  if((o.components?.liquidity||0)>=65)parts.push(tr('dobra płynność','good liquidity'));if(o.dest==='Black Market')parts.push('Black Market');
...
function filteredResults(){
  let a=[...state.opportunities],q=el('resultSearch').value.trim().toLowerCase(),minC=+el('minConfidence').value||0;if(q)a=a.filter(o=>[displayOpportunityName(o),o.itemNamePl,o.itemNameEn,o.itemId,o.source,o.dest,o.model].some(x=>String(x).toLowerCase().includes(q)));if(state.watchOnly)a=a.filter(o=>state.watched.has(o.itemId));a=a.filter(o=>(o.confidence??0)>=minC);
...
function render(){
  const a=filteredResults();el('emptyState').style.display=a.length?'none':'block';el('resultsBody').innerHTML=a.map((o,i)=>`<tr><td><button class="star ${state.watched.has(o.itemId)?'on':''}" data-watch="${esc(o.itemId)}">★</button></td><td><div class="itemcell"><img loading="lazy" src="${itemIcon(o.itemId,o.quality)}" alt=""><div><div class="itemname">${esc(displayOpportunityName(o))}</div><div class="itemid">${esc(o.itemId)} • ${qualityName[o.quality]||o.quality}</div></div></div></td><td>${modelTags(o)}</td><td class="route"><b>${esc(marketLabel(o.source))}</b> → <b>${esc(marketLabel(o.dest))}</b></td><td>${fmt(o.buy)}</td><td>${fmt(o.sell)}</td><td class="${o.profit>=0?'profit':'loss'}">+${fmt(o.profit)}</td><td class="${o.roi>=0?'profit':'loss'}">${pct(o.roi)}</td><td>${fmt(o.riskProfit)}</td><td class="${o.age<=3?'fresh':o.age<=24?'stale':'old'}">${ageText(o.age)}</td><td><b>${(o.confidence||0).toFixed(0)}</b><div class="scorebar"><i style="width:${clamp(o.confidence||0)}%"></i></div></td><td><b>${(o.score||0).toFixed(0)}</b></td><td>${flag(o)}</td><td><button class="btn" data-detail="${i}">${tr('Szczegóły','Details')}</button></td></tr>`).join('');
